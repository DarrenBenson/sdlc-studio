...
> **Severity:** Medium
> **Points:** 2
> **Affects:** tools/check_spec_claims.py, sdlc-studio/tsd.md, tools/tests/test_check_spec_claims.py
> **Created:** 2026-09-23
...
- [ ] **AC1** The behaviour described is corrected: The pre-commit spec-claims lane judges tsd.md's skill-tests <= 950s claim against the median of the clone's own machine-local timing store, and it refuses...
- [ ] **AC2** Following the recorded steps no longer reproduces the defect: In a fresh worktree, make one commit while other suites run so the first skill-tests sample exceeds 950s, then commit again: spec-claims refuses before any...
- [ ] **AC3** The proposed fix lands, pinned by a test: Delete the timing claim lane (it checks docs against docs), or judge timing claims only at the push boundary against the main clone's record.
