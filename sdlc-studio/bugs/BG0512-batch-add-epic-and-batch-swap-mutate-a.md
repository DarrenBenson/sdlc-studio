...
- [ ] The behaviour described is corrected: `sprint.py plan` refuses a batch holding an ungroomed unit, and since BG0511 that census covers bugs as well as stories.
- [ ] The proposed fix lands, pinned by a test: Route both verbs through the same `breakdown` census `plan --write` uses, and refuse on the same terms.
