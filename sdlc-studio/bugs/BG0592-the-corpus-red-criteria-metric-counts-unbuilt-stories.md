...
| AC11 | in `gate.py`, pass a project root to `status_vocab` in `_claims_completion`, so a project-declared status is classified by that project's own vocabulary | Given a project that DECLARES a story status in its `.config.yaml` and moves finished stories to it, when the lane classifies a red criterion on one, then it is COUNTED. |
