...
> **Status:** Fixed
> **Severity:** High
> **Verification depth:** functional [[derived: criteria 15; plan rows 18; executed 18; killed 17; survived 0; ruled equivalent 1; not-run 0; retracted 1; entry point 1 of 15 criteria through the shipped CLI, 14 in-process | fp 706d8b635774 ]] (fifteen criteria drive the real release lane over temp corpora at seven story statuses. AC15 drives it through the SHIPPED CLI by subprocess and is the only one that does - a review found this field claiming 'several through the shipped CLI' when every `Verify:` line resolved to a class whose entry point is `gate.run_gate(...)`, so the coverage the sentence described has been ADDED rather than the sentence softened. Mutation: 18 declared Test Plan rows across the 15 criteria - AC13 carries four, because subtracting a subset, counting manual criteria as green, and emitting the clause unconditionally are distinct ways for it to be wrong. `run --from-plan` joins one row per criterion, so it reports 15 of the 18: `verify_ac._testplan_rows` keys by criterion and silently drops the extras, which is the enumerated-list shape this repository keeps meeting and is filed separately. Every row was executed. Each anchor asserted unique, `__pycache__` purged and `python3 -B`, 18 executed with 17 KILLED and one ruled equivalent with its reason - AC13 row 2, whose `passing` subtraction is unreachable because both printing paths require an empty `blocked` by construction - plus one `equivalent` RETRACTED on the record when a review found the mutant it named unreachable by the fixture rather than by the code; restore byte-exact - TEN of them from three review rounds that rejected the first, second and third cuts, including one that reverted the entire repair in a single line while every test stayed green. Re-measured end to end through `verify-corpus.sh` in an isolated worktree: 20 red on Done of 87 in total, with the 67 excluded rows printed in full with their statuses. THREE false readings were taken and discarded before the baseline moved - 26, inflated by the author's own uncommitted style violation breaking a lane six criteria invoke; a per-AC 21 whose extra row was a load timeout; and an 88/68/21 partition carried into durable prose after the measurement had already corrected it to 87/67/20. This field itself carried a fourth such error, claiming ten criteria and thirteen mutants against twelve rows, and is rewritten from the artefact rather than from memory)
> **Points:** 3
...
| AC10 | in `gate.py`, replace the exclusion ledger with `_elide(...)`, truncating it to the first ten | Given more excluded criteria than the lane's elision cap, when it reports, then every one is named - a partial ledger of what was removed from the enforced number is the audit going missing. |
| AC11 | in `gate.py`, extend `_claims_completion`'s `status_vocab` call with a project root, so a project-declared status is classified by that project's own vocabulary instead of reading as unclassifiable and failing closed | Given a project that DECLARES a story status in its `.config.yaml` and moves finished stories to it, when the lane classifies a red criterion on one, then it is COUNTED. |
...
| AC13 | in `gate.py`, set the exclusion-only PASS green count to `executable` | Given a PASS reached through the exclusion ledger, when the lane states its green figure, then that figure EXCLUDES every failing class. |
| AC13 | in `gate.py`, narrow the subtraction to `red` | Subtracting a SUBSET is the same defect quieter. |
| AC13 | in `gate.py`, drop `len(blocked)` from the `passing` subtraction | A FAIL states no green figure at all. |
| AC13 | in `gate.py`, replace the green count with `acs - len(unbuilt)`, counting MANUAL criteria as green | The green figure counts only what executed AND passed. |
| AC15 | in `gate.py`, delete the `claims_done` guard and ask whether the shipped CLI still prints the exclusion verdict | Given the release gate driven as a subprocess, when the corpus carries a failure on a story claiming no completion, then the CLI prints it as excluded rather than red. |
