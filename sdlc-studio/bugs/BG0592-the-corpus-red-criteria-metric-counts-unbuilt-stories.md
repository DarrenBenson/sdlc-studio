...
| AC11 | in `gate.py`, replace the resolved root with `Path.cwd()` when classifying against `status_vocab` | Given a project that DECLARES a story status in its `.config.yaml` and moves finished stories to it, when the lane classifies a red criterion on one, then it is COUNTED. |
