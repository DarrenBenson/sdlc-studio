...
> **Severity:** High
> **Points:** 3
> **Verification depth:** functional (criteria red-first on both halves. CORRECTED 2026-08-01 after independent review: the original record claimed the backfill mirror was covered by a killed mutant. It was not - `remake` carries its own copy of the exemption and had NO test, and restoring `(idn or 0)` there SURVIVED the whole 5,658-test suite. Two tests now hold that half and the mutant is KILLED. Four mutants applied singly, purged, restored byte-identical - the `or 0` no-ordinal hole restored, the run window ignored, and the run-scoping predicate replaced with a constant: all KILLED. The predicate mutant SURVIVED a first attempt: every fixture was either unstamped or claimed by a span, so nothing separated `raised outside a batch` from `raised at all`. A finding stamped INTO a batch but unclaimed now pins that distinction - which is AC3 itself)
