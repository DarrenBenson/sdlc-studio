...
- [ ] **AC2** Given the same fixture with a PARTIAL repair, when the gate runs, then it still blocks AND its refusal names the outstanding findings. The message matters: a partial repair was blocked before this change too, by the plain REJECT path, so a criterion asserting only the block passes on pre-existing behaviour - naming what is still outstanding is computable only through the new consultation
...
- [ ] **AC3** Given a fixture whose `.config.yaml` sets `review.test_plan_after` so the gate is ACTIVE, when `transition.py set <unit> 'In Progress'` is run through the shipped CLI, then it SUCCEEDS for the repaired unit and REFUSES for an unrepaired one. Both halves are required: the gate stands down entirely when the cutoff is unset, which is the default, so a success asserted alone goes green on a gate that never fired
  ...
  - **Verified:** yes (2026-08-27)
- [ ] **AC4** Given a fixture in which a unit carries a DELIVERY repair and a PLAN-REVIEW rejection recorded on the SAME DATE, when the plan-review gate runs, then it still blocks. `repairs_for` takes no phase and the ledger has no phase column, so without this the gate this unit adds is opened by a repair from another phase entirely
  ...
  - **Verified:** yes (2026-08-27)
- [ ] **AC5** Given a fixture in which a unit carries MORE unanswered rejections on the repair's date than it has repair rows, when the gate runs, then it blocks - the cheap residual guard standing in for BG0631's join repair, which is a ledger schema change out of this unit's scope. A single repair must not discharge a day's worth of rejections
  - **Verify:** pytest .claude/skills/sdlc-studio/scripts/tests/test_transition.py::PlanReviewRepairGateTests::test_one_repair_does_not_discharge_a_days_worth_of_rejections
  - **Verified:** yes (2026-08-27)
- [ ] **AC6** Given an isolated fixture holding a DELIVERY rejection whose repair is complete but whose tier depth is NOT covered, when `critic.verdict_for` is read, then it still returns REJECT and conformance reaches its answer through `conformance.py`:355. This is the placement guard, stated as a property rather than as a before-and-after snapshot: relocating the consultation into `verdict_for` makes `per_unit_ok` true, runs `tier_covers`, and flips the answer
...
| AC5 | in `.claude/skills/sdlc-studio/scripts/transition.py`, drop the count comparison between the date's unanswered rejections and its repair rows, so one repair discharges every rejection sharing its date | Given a fixture in which a unit carries MORE unanswered rejections on the repair's date than it has repair rows, when the gate runs, then it blocks - the cheap residual guard standing in for BG0631's join repair, which is a ledger schema change out of this unit's scope. A single repair must not discharge a day's worth of rejections |
