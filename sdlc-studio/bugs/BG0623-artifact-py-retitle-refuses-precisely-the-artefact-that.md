...
> **Verification depth:** functional [[derived: criteria 4; plan rows 5; executed 5; killed 5; survived 0; not-run 0; entry point 4 of 4 criteria through the shipped CLI, 0 in-process | fp 60c4c73ab688 ]] (four criteria, five mutants - AC1 carries two, because its criterion names two distinct wrongs and one mutant cannot stand for both. Every one applied to the real file with bytecode purged and the tree restored. Entry point reads 4 of 4: every criterion drives `artifact.py retitle` as a subprocess, which is where the bug's own evidence lives. AC1's fixtures invoke with a NON-canonical spelling on purpose - `norm_id` folds case and the dash, so an implementation stamping the caller's spelling passes any test that asks canonically.)
> **Created:** 2026-08-26
