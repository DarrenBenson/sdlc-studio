...
> **Verification depth:** functional (all criteria drive the real close chain over temp git repos at four rungs and across both unit types, and the repair was confirmed end to end through the shipped verb: `sprint close --dry-run` on RUN-01M05A5M fell from 53 blockers to 8, of which 3 block. Mutation: 10 mutants, each anchor asserted to occur exactly once, `__pycache__` purged and `python3 -B`, all 10 KILLED, restore byte-exact. FOUR of the ten came from an adversarial review that REJECTED the first cut - a story-only fixture let a `type_ != "story"` skip survive all 895 tests in the file, a single-shape fixture let `story_is_ungroomed` substitute for `unit_is_ungroomed`, the scope was `!= "done"` rather than `== "design"` and so moved the defect onto the `plan` and `triage` rungs, and the blocking assertion tested a key the code never sets. Three of the ten exist only to catch over-correction, because the author of this repair was the session it unblocked)
