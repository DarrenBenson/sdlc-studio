...
through the other door. Both FILING writers are in scope here. "Nothing else writes it" was in this artefact and is false: a delivery review executed `transition.py annotate --id <bug> --field Severity --value major` and got exit 0 with `> **Severity:** major` on disk, because `severity` is absent from `_ANNOTATE_DENYLIST`. `transition.py` is outside this unit's Affects, so the third writer is filed as BG0633 rather than fixed here - but the claim is corrected, because a wrong claim in the record is the thing this batch exists to stop.

## Acceptance Criteria

- [x] **AC1** Given a finding whose Severity matches neither the barred nor the disclosed set, when the residue is read, then it is NAMED in the residue report AND is absent from `corpus()`'s mapping. Both halves: "reported rather than absent" is satisfied by the id merely appearing in output, and under the classify-as-disclosed mutant it does appear - on the page, as a disclosed finding
...
- [x] **AC2** Given a MEDIUM finding, when the population is read, then it is disclosed and NOT barred; and given a HIGH one, barred and not disclosed. The control names BOTH directions, because a mutant folding one reader into the other moves a recognised severity and an assertion about one side alone survives it
  ...
  - **Verified:** yes (2026-08-28)
- [x] **AC3** Given an unrecognised severity offered to `file_finding.py`, when the finding is filed, then it is REFUSED naming the accepted set - not normalised, which would guess at intent. The criterion and its selector state one behaviour between them
  ...
  - **Verified:** yes (2026-08-28)
- [x] **AC4** Given the SECOND writer, `artifact.py new --type bug`, when an unrecognised severity is offered, then it is refused the same way. Normalising one entry point leaves the class open through the other, so stopping the class is a claim only both together can make
  ...
  - **Verified:** yes (2026-08-28)
- [x] **AC5** Given the shipped commands, when `known_issues.py --bar` and `--check` are run as SUBPROCESSES over a fixture holding an unrecognised severity, then both NAME it and `--bar` still exits 0, because an unclassifiable severity is reported rather than barring. The Impact is stated entirely in terms of these two commands, and a repair widening the population functions while `--bar` prints `release bar met` passes every library row
  ...
  - **Verified:** yes (2026-08-28)
- [x] **AC6** Given a finding whose unrecognised severity sits on a CLOSED unit, when the residue is read, then it is still NAMED. The reader is deliberately status-blind: both classifying readers test open-ness BEFORE severity, so an unrecognised value on a closed unit is excluded twice over, and the corpus's only instance - BG0149, Fixed - is exactly that shape. Every fixture in the first cut filed at `Open`, so a mutant skipping closed findings passed all three rows while the defect returned in full on the real tree
  - **Verify:** pytest tools/tests/test_known_issues.py::UnclassifiableSeverityTests::test_a_closed_finding_with_an_unrecognised_severity_is_still_named
  - **Verified:** yes (2026-08-28)
- [x] **AC7** Given a severity written in the corpus's real spellings - lowercase, or padded with spaces - when the residue is read, then it is NOT reported: the residue reader folds case and trims exactly as the classifying readers do. The corpus holds 21 lowercase rows against fixtures that were all canonically spelled, so a case-sensitive residue reader passed the suite and turned the live report from one finding into twenty-two
  - **Verify:** pytest tools/tests/test_known_issues.py::UnclassifiableSeverityTests::test_the_residue_reader_folds_case_and_trims_like_the_classifiers
  - **Verified:** yes (2026-08-28)
- [x] **AC8** Given a severity offered to either writer with surrounding whitespace, when the finding is written, then it is accepted and stored canonically - trimmed at BOTH ends, not only the left. `known_issues._matches` strips both, so a corpus row whose severity carries a trailing space classifies perfectly well and a left-only writer refuses it. This is the case-folding argument applied to the other half of the same normalisation
  - **Verify:** pytest .claude/skills/sdlc-studio/scripts/tests/test_artifact.py::SeverityVocabularyTests::test_surrounding_whitespace_is_trimmed_at_both_ends
  - **Verified:** yes (2026-08-28)
- [x] **AC9** Given a RECOGNISED severity offered to either writer, when the artefact is written, then the value stored is the value REQUESTED in its canonical spelling - not merely a value equal to what the lowercase twin produced. A writer stamping a constant satisfies every same-input comparison, exits 0, and moves a Critical finding off the release bar and onto the disclosure page: the harm AC1 names, arriving through the writer rather than the reader
  - **Verify:** pytest .claude/skills/sdlc-studio/scripts/tests/test_artifact.py::SeverityVocabularyTests::test_the_written_severity_is_the_one_requested
  - **Verified:** yes (2026-08-28)
...
| AC3 | in `.claude/skills/sdlc-studio/scripts/file_finding.py`, replace the folded comparison with `raw == known`, so the 21 findings the live corpus spells `high`/`medium`/`low` fail the loop while their capitalised twins pass | Given an unrecognised severity offered to `file_finding.py`, when the finding is filed, then it is REFUSED naming the accepted set - not normalised, which would guess at intent. The criterion and its selector state one behaviour between them |
| AC3 | in `.claude/skills/sdlc-studio/scripts/file_finding.py`, change the loop predicate to `if False:` so the fall-through `raise` fires for every input. Named because the original control asserted only that `is not one of` was absent from output, which is equally true of a command exiting non-zero for a wholly unrelated reason - the grooming gate, in the event - and this mutant SURVIVED it | Given an unrecognised severity offered to `file_finding.py`, when the finding is filed, then it is REFUSED naming the accepted set - not normalised, which would guess at intent. The criterion and its selector state one behaviour between them |
...
| AC4 | in `.claude/skills/sdlc-studio/scripts/artifact.py`, replace the folded comparison with `raw == known`, so the 21 findings the live corpus spells `high`/`medium`/`low` fail the loop while their capitalised twins pass | Given the SECOND writer, `artifact.py new --type bug`, when an unrecognised severity is offered, then it is refused the same way. Normalising one entry point leaves the class open through the other, so stopping the class is a claim only both together can make |
| AC4 | in `.claude/skills/sdlc-studio/scripts/artifact.py`, change the loop predicate to `if False:` so the fall-through `raise` fires for every input. Named because the original control asserted only that `is not one of` was absent from output, which is equally true of a command exiting non-zero for a wholly unrelated reason - the grooming gate, in the event - and this mutant SURVIVED it | Given the SECOND writer, `artifact.py new --type bug`, when an unrecognised severity is offered, then it is refused the same way. Normalising one entry point leaves the class open through the other, so stopping the class is a claim only both together can make |
...
| AC6 | in `tools/known_issues.py`, add `not _is_open(row[1])` to the residue loop's `continue`, restoring the status test both classifying readers apply | Given a finding whose unrecognised severity sits on a CLOSED unit, when the residue is read, then it is still NAMED. The reader is deliberately status-blind: both classifying readers test open-ness BEFORE severity, so an unrecognised value on a closed unit is excluded twice over, and the corpus's only instance - BG0149, Fixed - is exactly that shape. Every fixture in the first cut filed at `Open`, so a mutant skipping closed findings passed all three rows while the defect returned in full on the real tree |
| AC7 | in `tools/known_issues.py`, replace the residue loop's `_matches(...)` calls with bare `in DISCLOSED` / `in BARRED` membership tests | Given a severity written in the corpus's real spellings - lowercase, or padded with spaces - when the residue is read, then it is NOT reported: the residue reader folds case and trims exactly as the classifying readers do. The corpus holds 21 lowercase rows against fixtures that were all canonically spelled, so a case-sensitive residue reader passed the suite and turned the live report from one finding into twenty-two |
| AC8 | in `.claude/skills/sdlc-studio/scripts/artifact.py`, replace `.strip()` with `.lstrip()` in `normalise_severity` | Given a severity offered to either writer with surrounding whitespace, when the finding is written, then it is accepted and stored canonically - trimmed at BOTH ends, not only the left. `known_issues._matches` strips both, so a corpus row whose severity carries a trailing space classifies perfectly well and a left-only writer refuses it. This is the case-folding argument applied to the other half of the same normalisation |
| AC8 | in `.claude/skills/sdlc-studio/scripts/file_finding.py`, replace `.strip()` with `.lstrip()` in `normalise_severity` | Given a severity offered to either writer with surrounding whitespace, when the finding is written, then it is accepted and stored canonically - trimmed at BOTH ends, not only the left. `known_issues._matches` strips both, so a corpus row whose severity carries a trailing space classifies perfectly well and a left-only writer refuses it. This is the case-folding argument applied to the other half of the same normalisation |
| AC9 | in `.claude/skills/sdlc-studio/scripts/artifact.py`, append `and 'Medium'` to the bug branch's severity expression, so the guard still runs and the stored value ignores it | Given a RECOGNISED severity offered to either writer, when the artefact is written, then the value stored is the value REQUESTED in its canonical spelling - not merely a value equal to what the lowercase twin produced. A writer stamping a constant satisfies every same-input comparison, exits 0, and moves a Critical finding off the release bar and onto the disclosure page: the harm AC1 names, arriving through the writer rather than the reader |
| AC9 | in `.claude/skills/sdlc-studio/scripts/artifact.py`, drop the vocabulary list from the refusal message, so the caller is told they are wrong without being told what is right | Given a RECOGNISED severity offered to either writer, when the artefact is written, then the value stored is the value REQUESTED in its canonical spelling - not merely a value equal to what the lowercase twin produced. A writer stamping a constant satisfies every same-input comparison, exits 0, and moves a Critical finding off the release bar and onto the disclosure page: the harm AC1 names, arriving through the writer rather than the reader |
