...
| 2026-07-31 | Claude Opus 5 | Rejoinder ruled findings 6-8 CLOSED and finding 9 MOVED: the authoriser limb held, but `scope_tail_error` stripped the scope tail while the subject normaliser stripped only the outer ends, so the VALIDATOR was more permissive than the STORE - `rule:sprint-checklist: cost` passed, was written with the space intact, and the lookup never found it. The waiver recorded cleanly, read as accepted, and covered nothing: verbatim the defect this AC claims to end. The normaliser now strips every segment. `_planned_points`' absent-versus-zero sentinel was also unpinned and now has a test. |
