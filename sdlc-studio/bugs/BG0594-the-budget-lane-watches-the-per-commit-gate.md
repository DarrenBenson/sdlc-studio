...
| AC5 | in `gate_timing.py`, drop the fallback label from the detail string | Given a FULL run and no declared full-suite figure, when the lane reports, then it judges against the per-commit ceiling as before AND says so, naming `gate_budget.full_seconds` as the key that would judge it on its own terms - going silent would drop the budget line entirely for every project that declares only `seconds` and never runs selected, and an UNLABELLED mis-comparison is the defect this bug is about |
