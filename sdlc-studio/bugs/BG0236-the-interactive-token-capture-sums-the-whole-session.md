...
| 2026-07-21 | claude | Fixed: `open_run` stamps a session-token baseline on a fresh mint; the close captures the DELTA. No fallback to the raw total - a run with no baseline, or one closed from another session, reports NOT-ATTRIBUTABLE. 7 mutants killed. |
| 2026-07-21 | claude | Repair after adversarial REJECT (round 1). Two findings against this unit, both in the reader rather than the baseline logic. (1) MAJOR: `session_tokens` summed `int(usage.get(k) or 0)` under an OSError-only clause, so one non-numeric usage value raised TypeError out of `_session_baseline` and out of `open_run` - `sprint plan --write` minted NO run and wrote no run-state.json, while both docstrings promised the opposite. The clause is now the whole family, matching what `archived()._index` in the same module had already learned; a malformed record REFUSES the total rather than skipping the record, because a short total inflates every delta measured against it. (2) MINOR: the cross-session guard read `base.source and cap.source and base != cap`, so a baseline with no source disabled the guard - and an unsourced baseline is precisely the one that cannot be shown to be this meter's. It now refuses. 4 further mutants killed, one of which SURVIVED first time (the `_session_baseline` backstop was unreachable through the public path, L-0159) and is now pinned directly. |
