...
| AC6 | in `sprint.py`, wrap the ledger read and fall through on error | Given a DIRECTORY at the critic ledger's path - a shape that raises for every user, where `chmod 000` is a no-op when the suite runs as root - when `_built_not_closed` judges, then it fails closed and treats the unit as unbuilt; a MISSING ledger is the no-verdict case and must not satisfy this criterion |
