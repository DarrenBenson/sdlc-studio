...
- [x] **AC8: a sign-off is not recorded against a unit that has not been delivered.**
  ...
  - **Verify:** manual - retired by US0919: `critic.py signoff` is retired and the operator signs the run once at `sprint sign`, so no per-unit sign-off row is written to withhold
...
| 2026-09-25 | US0919 | AC8 retired in the D0259 pattern: `critic.py signoff` is retired and the operator signs the run once at `sprint sign`, so no per-unit sign-off row is written to withhold |
