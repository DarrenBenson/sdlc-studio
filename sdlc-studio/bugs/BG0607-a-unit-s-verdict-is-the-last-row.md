...
>
> **A THIRD direction WORKS, and the two above were nested rather than independent.** An adversarial review refuted the conclusion this note first carried. The ledger already stores a partial round identifier: the `Brief` column, a content hash of the brief the seat was handed (`critic.py:143`), which embeds the seat charter and so cannot conflate two seats - `_seats_whose_brief_matches` at `critic.py:110` exists to read it back. Keying the retraction on a MATCHING FINGERPRINT, with no heuristics, recovers a strict superset of what the reviewer string recovers: measured on this corpus, the fingerprint rule recovers 32 units that the reviewer-string rule leaves unanswered, and loses none - the RECOVERY is the reproducible figure and the one the fix rests on. The absolute counts depend on the scope counted and on the tie-break, so they are stated only where both are stated: under the latest-unanswered rule, the shipped conformance lane goes from 608/690 with 0 non-conformant to 589/690 with 19. US0593 is the shape: `engineering; delivery review subagent; round 1` REJECT and `engineering; final independent pass; RUN-01KZCAJX` APPROVE, different strings, identical fingerprint `b8c97c1ab63b` - the round, stored, today.
>
...
> **Affects:** .claude/skills/sdlc-studio/scripts/critic.py, .claude/skills/sdlc-studio/scripts/tests/test_critic.py, .claude/skills/sdlc-studio/scripts/transition.py, .claude/skills/sdlc-studio/scripts/tests/test_transition.py, docs/known-issues.md, docs/release-notes-v5.0.1.md, tools/tests/test_known_issues.py, tools/tests/check_backfill_evidence.py, tools/tests/test_check_backfill_evidence.py, sdlc-studio/reviews/repair-record.md
...
- [ ] **AC1** Given a unit with a REJECT from one seat and an APPROVE from another, when the standing verdict is read, then it is the LATEST unanswered REJECT - a panel is several seats recorded one after another, and taking the last row written made the verdict a fact about the order the recorder was called in. The tie-break is stated because it is load-bearing: reporting the EARLIEST unanswered rejection instead yields 18 non-conformant units rather than 19, and the unit it drops is US0671 - the first unit this bug's own Steps to Reproduce names as masked
...
- [ ] **AC3** Given the fingerprint-keyed roll-up, when the shipped whole-workspace conformance lane runs, then it is GREEN - this roll-up shipped once and was withdrawn because it took the lane from 608/690 with 0 non-conformant to 579/690, and the fingerprint rule alone still leaves 18. It lands with BG0620 or it does not land
  - **Verify:** pytest .claude/skills/sdlc-studio/scripts/tests/test_critic.py::LedgerRollupTests::test_the_workspace_conformance_lane_is_green_under_the_new_rule
- [ ] **AC4** Given this is the LAST open High to close, when it reaches Fixed, then the disclosure surfaces are true in the same commit: `docs/known-issues.md` regenerated, and `docs/release-notes-v5.0.1.md` carrying the claim the guard demands for an empty barred set - reaching the bar FALSIFIES both pages, and `test_known_issues.py` goes red at the moment the bar is met unless they move with it
  - **Verify:** pytest tools/tests/test_known_issues.py
- [ ] **AC5** Given the new roll-up in force, when every one of the NINETEEN units carrying an unanswered REJECT is backfilled - US0577, US0578, US0580, US0583, US0585, US0591, US0597, US0629, US0630, US0631, US0632, US0645, US0662, US0663, US0664, US0665, US0666, US0671, US0676 - then each reads a `complete` repair state and every closure names the evidence that closed its finding. The code lands FIRST and the backfill second: `record_repair` refuses a unit with no live REJECT, and under the shipped roll-up all nineteen read APPROVE, so the backfill is not recordable until this roll-up ships
  - **Verify:** shell python3 tools/tests/check_backfill_evidence.py
- [ ] **AC6** Given the backfill is complete, when every finding across those nineteen units is counted, then each is EITHER closed with named evidence OR filed as an artefact, and the split is REPORTED as two numbers. This replaces asking whether an unrepaired finding exists, which only the author can answer and which asserts nothing when the author answers no
  - **Verify:** pytest tools/tests/test_check_backfill_evidence.py::PositiveControlTests::test_the_split_of_closed_versus_filed_is_reported
- [ ] **AC7** Given a FIXTURE whose repair record leaves one finding closed by nothing, when the backfill checker runs against it, then the checker FAILS and names that finding - the checker is written by the unit it grades, so without a case it must reject, a script that prints ok and exits 0 satisfies AC5 and AC6 exactly as well as a real one
  - **Verify:** pytest tools/tests/test_check_backfill_evidence.py::PositiveControlTests::test_the_checker_fails_on_a_finding_closed_by_nothing

## Test Plan

| Criterion | Mutant - the production change this test must fail on | Title |
| --- | --- | --- |
| AC1 | in `critic.py`, return the last live row from `verdict_for` instead of an unanswered REJECT | Given a unit with a REJECT from one seat and an APPROVE from another, when the standing verdict is read, then it is the LATEST unanswered REJECT - a panel is several seats recorded one after another, and taking the last row written made the verdict a fact about the order the recorder was called in. The tie-break is stated because it is load-bearing: reporting the EARLIEST unanswered rejection instead yields 18 non-conformant units rather than 19, and the unit it drops is US0671 - the first unit this bug's own Steps to Reproduce names as masked |
