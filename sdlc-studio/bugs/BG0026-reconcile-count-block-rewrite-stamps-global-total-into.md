...
`reconcile --apply` (called by gate/autosprint/project upgrade) recomputes index summary counts. An index can carry many `| Status | Count |` tables (per-epic roll-ups) plus one global summary; the old code stamped the GLOBAL total into EVERY one. On agent-crew it overwrote each per-epic `| Done | 6 |` with the fleet total (590) - silent index corruption, caught only by diffing before commit.
