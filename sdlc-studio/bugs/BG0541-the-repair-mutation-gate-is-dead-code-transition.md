...
| 2026-08-07 | sdlc-studio | Delivery review round 1 REJECTed on two blocking findings, both established by execution. An UNRESOLVABLE base ref - not just an absent one - granted a false exemption, because `changed_lines` swallows a failed `git diff` and returns an empty map, so no mutant read as the claim holding: AC7's own defect one condition over, reachable from a SHA lost to an amend. And AC2's `config-defaults.yaml` clause had never shipped while the criterion read Verified. Both fixed, the AC2 test now pins the key, `verify_no_surface_claim`'s dead `text` parameter is read and names the declared scope beside the derived one, `off` returns before doing the work it was discarding, and the unreadable-ledger arm refuses rather than passing |
| 2026-08-07 | sdlc-studio | Delivery review round 2 APPROVEd, ruling all three round-1 blocking findings CLOSED by execution and one OVER-CLAIMED - the vacuous exemption test had not been touched. It now discriminates: the SAME markdown-only diff proceeds with a record and is refused without one, so the record decides the outcome rather than being read back from the fixture that planted it. `ledger_entries` raises rather than reading a REPLACED ledger as an empty history, and the `off` stand-down is pinned by a test that asserts the re-derivation is never reached |
