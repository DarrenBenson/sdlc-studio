...
- [x] **AC2** Given a run that examined at least one unit, when the detail is rendered, then it reports the count and the outcome as it does today - the control, without which reporting an absence unconditionally satisfies the row above
