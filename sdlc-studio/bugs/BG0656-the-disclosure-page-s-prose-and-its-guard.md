...
> **Affects:** tools/known_issues.py, tools/tests/test_known_issues.py, docs/known-issues.md, docs/release-notes-v5.1.md
...
Repoint the guard at the release being cut (`docs/release-notes-v5.1.md`), so a released version's record freezes and the guard tracks the current one. Derive the page's prose from its own row count so it is true at N and at zero, and state the v5.1 bar on the page with v5.0.0's kept as history.
