...
| Suite size | Several thousand tests across 90+ modules, and MINUTES rather than seconds - the recorded runs sit around 700-800s for the skill suite (the median rose to about 780s in RUN-01M2JA6J, whose commits ran beside many parallel agents) and 9-12s for the tools suite <!-- measured: skill-tests <= 820s --> <!-- measured: tool-tests <= 250s -->. Run the discover command for the live count and `tools/gate_timing.py estimate` for the live duration rather than trusting a pinned number: both drift every sprint, and the cost is why the hook skips the suites for a commit that cannot reach them |
