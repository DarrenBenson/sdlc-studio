...
| Suite size | Several thousand tests across 90+ modules, and MINUTES rather than seconds - the recorded runs sit around 700-730s for the skill suite and 9-12s for the tools suite <!-- measured: skill-tests <= 760s --> <!-- measured: tool-tests <= 250s -->. Run the discover command for the live count and `tools/gate_timing.py estimate` for the live duration rather than trusting a pinned number: both drift every sprint, and the cost is why the hook skips the suites for a commit that cannot reach them |
