# EP0170: The shipped surface is the documented surface: grooming, the open run and the run lifecycle

> **Status:** Done
> **Parent:** CR0442
> **Parent:** CR0440
> **Derived Point Total:** 13
> **Parent:** CR0439
> **Created:** 2026-07-27
> **Created-by:** sdlc-studio new
> **Raised-by:** sdlc-studio; agent; v1
> **Size:** S

## Summary

Decomposed from CR0439. Delivers the work CR0439 requested.

## Story Breakdown

- [x] [US0466: The ungroomed AC marker routes to the shape and the verifier guidance, help/refine.md ships, and the help-page gap is a derived lane](../stories/US0466-the-ungroomed-ac-marker-routes-to-the-shape.md)
- [x] [US0467: status names the open run - id, rung, Sprint Goal, batch and remaining - from the run state, with remaining derived from the same predicate the handoff uses](../stories/US0467-status-names-the-open-run-id-rung-sprint.md)
- [x] [US0468: help/sprint.md documents the run lifecycle - batch mutation, stop, appetite and rolling - bound in invocation form to the shipped parser and the run record](../stories/US0468-help-sprint-md-documents-the-run-lifecycle-batch.md)

## Revision History

| Date | Author | Change |
| --- | --- | --- |
| 2026-07-27 | sdlc-studio | Created via `new` (deterministic) |
