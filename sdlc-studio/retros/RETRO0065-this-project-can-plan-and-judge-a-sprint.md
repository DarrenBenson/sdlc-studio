...
- **An estimator that prices only the build will be wrong by whatever the proving costs.** The
  forecast said 400,000 and the sprint cost at least 4,119,916; the build was 787,834 of that and
  the review, repair and re-verification were most of the rest. The constant is not stale, it is
  measuring a different activity from the one this project performs.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BG0244 | 2 | 50,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0246 | 2 | 50,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0242 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0245 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0240 | 2 | 50,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0241 | 2 | 50,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0243 | 2 | 50,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| **Batch (rated units only)** | **0** | **0** | **0** | - | **-** | | **-** | - |

**0 of 7 unit(s) measured; 7 of 7 forecast at plan time.**

**Sprint tokens/point: 146,336** (2,634,055 tokens over 18 delivered points, harness-tracked). The token count is deterministic (supply it with `accuracy --tokens N`) - not UNMEASURED. A descriptive velocity, never a target.

**Velocity: 9.51 points/elapsed-hour** (18 points over 1.893h, run-state, ceremony included). This is the planning number - points per SESSION within the observed single-session envelope; it is NOT a linear per-point rate to extrapolate to a 1-point or 100-point sprint, and it is descriptive, never a target.

  secondary (points/worker-hour): UNMEASURED - no runner worker-time records (an interactive sprint has none).
Unmeasured: BG0244, BG0246, BG0242, BG0245, BG0240, BG0241, BG0243. They are excluded from the batch ratio - an unmeasured unit is not evidence that the estimate was right.
...
| The forecast prices only the BUILD, so it under-forecast this sprint by over 10x: 400,000 predicted against at least 4,119,916 spent, with the build itself only 787,834 of it | BG0254 |
...
- **FINAL AT SIGN-OFF: 2,634,055 published, at least 4,119,916 actually spent.** The mid-sprint
  reading of 439,982 was taken before the review, repair and re-verification rounds, which
  dominate the total. Excluded from the published figure: the four cluster agents (787,834),
  review round 1 (216,165), the repair round (191,969) and review round 2 (289,893) - 1,485,861
  of delegated spend the capture cannot see (BG0252).
- **THE SEED CONSTANT IS LOW BY AN ORDER OF MAGNITUDE, and this is the sprint's most useful
  number.** At 18 points the published rate is 146,336/pt and the true rate at least 228,884/pt,
  against a seed of 25,000 - **5.9x and 9.2x respectively**. The plan forecast 400,000 tokens and
  the sprint cost at least 4,119,916, over 10x. Most of the excess is the review and repair rounds,
  which the forecast does not model at all: it prices the BUILD and this project spends more on
  proving the build correct than on producing it. That is not an argument for spending less on
  review - the review caught three MAJORs including one that made a gate reward silence - but the
  estimator currently describes a way of working this project does not use. BG0248 asks where the
  rate should be measured from; this says what it will find when it looks.
- **Superseded mid-sprint reading: 439,982 published, at least 1,227,816 actually spent.** The capture works and counts
