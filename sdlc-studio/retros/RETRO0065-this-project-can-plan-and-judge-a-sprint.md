...
- **This is the first sprint opened WITH a session-token baseline**, stamped at plan time by
  BG0236's fix from the previous run. So unlike the last three closes, the token cost should be a
  real attributable delta rather than not-attributable. Whether it actually is, is the single most
  interesting number this close produces, and it is the previous sprint's work being judged.
