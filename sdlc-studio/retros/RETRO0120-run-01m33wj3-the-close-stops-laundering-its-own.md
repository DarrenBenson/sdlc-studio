...
The header reads 5 of 5 because the batch IS five: the six stories were dropped under D0250
BEFORE this close, so they are not units this run failed to deliver - they are units it no
longer held. The run was PLANNED at eleven, and that is the number the forecast is judged
against, recorded in the plan and repeated here so the denominator cannot quietly shrink to
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BG0733 | 2 | 97,240 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0715 | 2 | 97,240 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0730 | 3 | 145,860 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0719 | 3 | 145,860 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0722 | 3 | 145,860 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| **Batch (rated units only)** | **0** | **0** | **0** | - | **-** | | **-** | - |

**0 of 5 unit(s) measured; 5 of 5 forecast at plan time.**

**Sprint tokens/point: 272,443** (9,807,942 tokens over 36 delivered points, harness-tracked). The token count is deterministic (supply it with `accuracy --tokens N`) - not UNMEASURED. A descriptive velocity, never a target.
...
Review passes, by phase - read from the two verdict ledgers:
  ...
  test-plan review: 5 pass(es) over 5 unit(s), 0 rejected

  code review: 5 pass(es) over 5 unit(s), 0 rejected

  ratio: 1.00 code-review pass(es) per test-plan pass - the claim EP0207 is judged on, as a number
Unmeasured: BG0733, BG0715, BG0730, BG0719, BG0722. They are excluded from the batch ratio - an unmeasured unit is not evidence that the estimate was right.
...
- [HO-0081](../handoffs/HO0081-run-01m33wj3-closed-partial.md) - 0 remaining item(s): 0 copilot-tail, 0 judgement. Pick up with `sprint plan --worklist sdlc-studio/.local/handoff-worklist.txt`.
