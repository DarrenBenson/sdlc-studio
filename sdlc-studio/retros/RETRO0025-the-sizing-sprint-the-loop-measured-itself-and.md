...
| --- | --- | --- | --- | --- | --- |
| CR0257 | 29 | 67,400 | 97,863 | 0.69x | 483s |
| CR0258 | 45 | 77,000 | 107,623 | 0.72x | 690s |
| BG0132 | 39 | 73,400 | 129,957 | 0.56x | 725s |
| CR0259 | 29 | 67,400 | 144,711 | 0.47x | 772s |
| CR0260 | 29 | 67,400 | 162,204 | 0.42x | 1137s |
| **Batch (rated units only)** | | **352,600** | **642,358** | **0.55x** | **3807s** |
...
The forecast below is the one recorded at PLAN TIME, before any of this sprint's code existed. When
this retro was written it was NOT the number `retro.py accuracy` printed: that tool re-derived each
estimate from the live constants, so the table had to be kept by hand to preserve what had actually
been predicted. BG0133 fixed that - the generated block above now reads the recorded forecast and
agrees with this table to the token. It is kept for the tool-use column, which the analysis below
turns on.
