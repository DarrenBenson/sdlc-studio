...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BG0215 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| BG0218 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0277 | 2 | 50,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0278 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0279 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0280 | 5 | 125,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0281 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0282 | 5 | 125,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| US0283 | 3 | 75,000 | - | **UNMEASURED** (no telemetry token record) | - | - | - | - |
| **Batch (rated units only)** | **0** | **0** | **0** | - | **-** | | **-** | - |

**0 of 9 unit(s) measured; 9 of 9 forecast at plan time.**

**Sprint tokens/point: 79,687** (2,390,624 tokens over 30 delivered points, harness-tracked). The token count is deterministic (supply it with `accuracy --tokens N`) - not UNMEASURED. A descriptive velocity, never a target.

**Velocity: 10.82 points/elapsed-hour** (30 points over 2.772h, run-state, ceremony included). This is the planning number - points per SESSION within the observed single-session envelope; it is NOT a linear per-point rate to extrapolate to a 1-point or 100-point sprint, and it is descriptive, never a target.

  secondary (points/worker-hour): UNMEASURED - no runner worker-time records (an interactive sprint has none).
Unmeasured: BG0215, BG0218, US0277, US0278, US0279, US0280, US0281, US0282, US0283. They are excluded from the batch ratio - an unmeasured unit is not evidence that the estimate was right.
...
- The plan forecast 750,000 tokens; the harness-captured actual is 2,390,624 over 30 points -
  the 25k/pt seed under-forecast this Fable-delivered interactive sprint by ~3.2x, and the
  measured 79,687/pt sits ~1.6x above the operator's Opus heuristic (~50k/pt, RETRO0028).
  Elapsed velocity 10.82 pts/hour (2.772h ceremony included) matches the operator's "Fable
  takes about double Opus's hour per 20 points" feel almost exactly. First calibrated Fable
  row; CR0373 (model on the row) keeps it from ever pooling with Opus. One sprint fits noise -
...
| record_velocity writes the retro id verbatim, so the scaffold's dashed form minted a history-invisible row and the tail re-captured instead of reusing | BG0226 (filed at close; row healed under the canonical id) |
...
- [HO-0014](../handoffs/HO0014-the-sprint-s-instruments-tell-the-truth-and.md) - 7 remaining item(s): 0 copilot-tail, 7 judgement. Pick up with `sprint plan --worklist sdlc-studio/.local/handoff-worklist.txt`.
