...
| D2 | Do we ever RETIRE a test, and if so is measured mutation kill-yield the only sanctioned criterion (D)? | Resolved: Yes, a test may be retired, and measured mutation kill-yield with equivalent mutants excluded is the ONLY sanctioned criterion - SEQUENCED, not yet built. Human judgement of redundancy is explicitly forbidden as a criterion because this codebase falsifies it in both directions: BG0210's `type_ != "epic"` guard was covered by nothing across 3,180 tests, in_dev_repo's second clause survived the whole 197-test gate suite, and the depth-parity branch survived all 3,238 - while four tests in one session passed for incidental reasons, one of them written specifically to pin a defect it could not reach. So 'looks redundant' and 'looks like coverage' are both unreliable and only kill-yield decides. Equivalents must be excluded per BG0212, where 6 of 15 survivors were equivalent mutants; a retirement criterion built on raw survivor counts would delete tests that are pulling their weight. CR0363 (the RFC's stated prerequisite) is DELIVERED, so the manufactured-survivor hole is closed, but the enumeration is NOT authorised to be built yet: at ~40s of suite per mutant a full per-module enumeration is many multiples of the measured ~40 min close run. TRIGGER: build it only after CR0377 (derive the minimal covering test command from the selection scan, halving wall-time per mutant) and BG0238 (per-unit mutation evidence is never captured) have landed. This is the same shape as D6's earlier 'sequenced, not yet numbered' ruling - D2 stays live with those two as its trigger, it is not deferred indefinitely. -> CR0377, BG0238 |
...
| D4 | Does a `test` audit lens profile earn its place (E), or is G sufficient? | Resolved: Yes - option E earns its place. A 'test' audit lens profile is adopted as the qualitative backstop to G, because G is deterministic but structurally blind to the exact class this project keeps shipping: prose that misdescribes what the code or the test actually does. A mutant cannot detect a docstring that lies. The evidence is three sprints running (L-0146, L-0173): at RUN-01KY1WCR two of the three MAJORs were false claims written in prose to justify the author's own code - a comment saying 'same answer as before' when the narrowed except clause had made a shipped library path raise, and a hook comment saying 'the expensive lanes ran either way' when they had not, which published a -85% drift that was not drift. Round 2 of the same review then killed two replacement tests that both took the wrong branch, in a test whose own docstring claimed 'the behaviour is the claim'. G caught none of these and could not have. Lenses to build, drawn from the failure modes actually observed: can this test fail (mutate the guard it names); does it reach the code it claims to; does its docstring describe what it asserts; is it green for an incidental reason. On demand, not a new close-time ceremony - option F stays rejected because paying for test quality in review rounds is the currency this project is shortest of. |
| D5 | Does archival apply here? Index archival exists for artefacts (`archive.py`, `indexes.archive_after`) and four indexes are over the threshold. Is there an equivalent for tests, or is that a category error? | Resolved: Category error - D5 collapses into D2 and needs no separate mechanism. Index archival works because an index row is a POINTER: archive.py moves the row, the file stays put, the census is unaffected and nothing about what is true changes, which is why four indexes can be over threshold without any risk. A test is not a pointer, it is executable evidence, and archiving one has only two possible meanings: it still runs, in which case nothing is saved and the archive is decoration, or it stops running, which IS retirement under a gentler name and is therefore governed by D2's kill-yield criterion and D2's sequencing. There is no third state. The adjacent idea - a slow lane that runs at close and on CI but not per commit - is RFC0048 option A (tier the suite), which D1 considered and did NOT take: option B was chosen and delivered instead, taking a commit from 196.7s to 99s with no coverage given up. Reopening tiering here would relitigate D1 without new evidence. Resolve by pointing at D2. -> RFC0048 |
| D6 | What is the acceptable per-commit gate budget? Without a target number, every future guard is individually justifiable and the ratchet never stops | Resolved: 120 seconds per code commit, declared against a baseline of 99s on 2026-07-21 - the first END-TO-END total the hook recorded itself - held in sdlc-studio/.config.yaml as gate_budget with that baseline beside it. The component sum (skill-tests 78.1s + gate 6.95s + guards 8.0s = 93.1s) is recorded for reference only: the adversarial review of RUN-01KY1WCR showed it was hand arithmetic over three separately timed runs while the hook was recording 99s and 83s for the same work, and a baseline the check cannot itself reproduce is not a baseline. KNOWN LIMIT: same-machine variance measured ~19%, comparable to the ~28% ratchet the lane exists to expose, so the series is the signal and no single reading is. Pre-EP0093 the same commit cost 196.7s, so the budget sits below what a commit cost two days earlier. ~21% headroom over 99s: enough to absorb machine variance without the lane becoming noise that gets scrolled past, tight enough that a jump the size of test_gate.py's recent +28% trips it. ADVISORY ALWAYS, never blocking - a wall-clock check on a loaded or shared machine must not refuse a correct commit, the same call D3 made for mutation. The check reports the TREND against the dated baseline rather than a bare pass/fail, because reporting only 'under budget' is exactly how test_gate.py grew 56.4s to 72.2s in two days unnoticed: it was under every ceiling the whole time. It reads the LATEST recorded run, not the median, since a median over a ten-run window still read ~152s two commits after the suite had dropped to 79s. Implemented in tools/gate_timing.py and .githooks/pre-commit rather than the shipped gate.py: the hook machinery is repo-only, so a shipped lane would be permanently silent in every consuming project. -> US0287 |
...
| 2026-07-21 | rfc resolve (operator decision) | resolve: D6 - 120 seconds per code commit, declared against a baseline of 99s on 2026-07-21 - the first END-TO-END total the hook recorded itself - held in sdlc-studio/.config.yaml as gate_budget with that baseline beside it. The component sum (skill-tests 78.1s + gate 6.95s + guards 8.0s = 93.1s) is recorded for reference only: the adversarial review of RUN-01KY1WCR showed it was hand arithmetic over three separately timed runs while the hook was recording 99s and 83s for the same work, and a baseline the check cannot itself reproduce is not a baseline. KNOWN LIMIT: same-machine variance measured ~19%, comparable to the ~28% ratchet the lane exists to expose, so the series is the signal and no single reading is. Pre-EP0093 the same commit cost 196.7s, so the budget sits below what a commit cost two days earlier. ~21% headroom over 99s: enough to absorb machine variance without the lane becoming noise that gets scrolled past, tight enough that a jump the size of test_gate.py's recent +28% trips it. ADVISORY ALWAYS, never blocking - a wall-clock check on a loaded or shared machine must not refuse a correct commit, the same call D3 made for mutation. The check reports the TREND against the dated baseline rather than a bare pass/fail, because reporting only 'under budget' is exactly how test_gate.py grew 56.4s to 72.2s in two days unnoticed: it was under every ceiling the whole time. It reads the LATEST recorded run, not the median, since a median over a ten-run window still read ~152s two commits after the suite had dropped to 79s. Implemented in tools/gate_timing.py and .githooks/pre-commit rather than the shipped gate.py: the hook machinery is repo-only, so a shipped lane would be permanently silent in every consuming project. |
| 2026-07-21 | rfc resolve (operator decision) | resolve: D2 - Yes, a test may be retired, and measured mutation kill-yield with equivalent mutants excluded is the ONLY sanctioned criterion - SEQUENCED, not yet built. Human judgement of redundancy is explicitly forbidden as a criterion because this codebase falsifies it in both directions: BG0210's `type_ != "epic"` guard was covered by nothing across 3,180 tests, in_dev_repo's second clause survived the whole 197-test gate suite, and the depth-parity branch survived all 3,238 - while four tests in one session passed for incidental reasons, one of them written specifically to pin a defect it could not reach. So 'looks redundant' and 'looks like coverage' are both unreliable and only kill-yield decides. Equivalents must be excluded per BG0212, where 6 of 15 survivors were equivalent mutants; a retirement criterion built on raw survivor counts would delete tests that are pulling their weight. CR0363 (the RFC's stated prerequisite) is DELIVERED, so the manufactured-survivor hole is closed, but the enumeration is NOT authorised to be built yet: at ~40s of suite per mutant a full per-module enumeration is many multiples of the measured ~40 min close run. TRIGGER: build it only after CR0377 (derive the minimal covering test command from the selection scan, halving wall-time per mutant) and BG0238 (per-unit mutation evidence is never captured) have landed. This is the same shape as D6's earlier 'sequenced, not yet numbered' ruling - D2 stays live with those two as its trigger, it is not deferred indefinitely. |
| 2026-07-21 | rfc resolve (operator decision) | resolve: D4 - Yes - option E earns its place. A 'test' audit lens profile is adopted as the qualitative backstop to G, because G is deterministic but structurally blind to the exact class this project keeps shipping: prose that misdescribes what the code or the test actually does. A mutant cannot detect a docstring that lies. The evidence is three sprints running (L-0146, L-0173): at RUN-01KY1WCR two of the three MAJORs were false claims written in prose to justify the author's own code - a comment saying 'same answer as before' when the narrowed except clause had made a shipped library path raise, and a hook comment saying 'the expensive lanes ran either way' when they had not, which published a -85% drift that was not drift. Round 2 of the same review then killed two replacement tests that both took the wrong branch, in a test whose own docstring claimed 'the behaviour is the claim'. G caught none of these and could not have. Lenses to build, drawn from the failure modes actually observed: can this test fail (mutate the guard it names); does it reach the code it claims to; does its docstring describe what it asserts; is it green for an incidental reason. On demand, not a new close-time ceremony - option F stays rejected because paying for test quality in review rounds is the currency this project is shortest of. |
| 2026-07-21 | rfc resolve (operator decision) | resolve: D5 - Category error - D5 collapses into D2 and needs no separate mechanism. Index archival works because an index row is a POINTER: archive.py moves the row, the file stays put, the census is unaffected and nothing about what is true changes, which is why four indexes can be over threshold without any risk. A test is not a pointer, it is executable evidence, and archiving one has only two possible meanings: it still runs, in which case nothing is saved and the archive is decoration, or it stops running, which IS retirement under a gentler name and is therefore governed by D2's kill-yield criterion and D2's sequencing. There is no third state. The adjacent idea - a slow lane that runs at close and on CI but not per commit - is RFC0048 option A (tier the suite), which D1 considered and did NOT take: option B was chosen and delivered instead, taking a commit from 196.7s to 99s with no coverage given up. Reopening tiering here would relitigate D1 without new evidence. Resolve by pointing at D2. |
