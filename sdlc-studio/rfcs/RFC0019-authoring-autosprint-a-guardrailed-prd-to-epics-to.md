...
| D9 | Rename `autosprint` -> `sprint` (operator, 2026-06-24) | **Resolved: yes, as WS0.** The command is now the whole sprint lifecycle (`plan`/`design`/`done`), not just autonomous delivery; autonomy is the existing `--autonomous` *flag*, not the name. `sprint` is cleaner + standard agile vocabulary. Keep `autosprint` as a **deprecated alias** (NL resolver + a thin shim) so nothing breaks; history (CHANGELOG, closed CRs, RFC0001) keeps the old name. Do it **first** so new docs are written against the final name |
