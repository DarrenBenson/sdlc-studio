...
- [ ] The cascade leads with a single mechanical step naming artifact.py close --id US<NNNN> (or transition.py set) and tells the agent NOT to hand-edit the Status, index rows, summary counts, or epic checkbox
- [ ] The remaining narrated steps are limited to work the scripts do not cover and are labelled as residue, not the primary mechanism
- [ ] The #story-completion-cascade anchor is preserved and check_links.py still passes (referenced from reference-code.md/story.md/reconcile.md)
