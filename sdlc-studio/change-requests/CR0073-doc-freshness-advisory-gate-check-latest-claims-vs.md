...
- [ ] new `scripts/doc_freshness.py` compares LATEST.md's claimed version / test count / disclosure count against reality (SKILL.md version, the `def test_` census, `disclosure.check`)
- [ ] registered in `gate.py` DEFAULT_CHECKS as **advisory** (non-blocking), skill-only (no-op for consuming projects)
- [ ] tests + reference-scripts.md entry (doc-coverage); the check passes against the freshly-updated LATEST.md
