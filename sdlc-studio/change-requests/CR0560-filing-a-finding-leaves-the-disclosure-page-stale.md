...
| 2026-09-21 | audit ruling | still wanted, never started - returned to Proposed, and the tree is stale AT THIS MOMENT. `file_finding.py` contains no reference to the disclosure page and makes no subprocess call, so an operator must run `tools/known_issues.py --write` separately - and this run proved it twice, filing BG0721 and BG0722 and leaving the page disagreeing with the corpus until it was regenerated by hand. All three EP0245 children are Draft. |
