...
- **No test asserts a full-template story's header links its epic and persona**: US0081 AC1 (Done) says the full-template story header matches templates/core/story.md (linked epic, persona), but no test asserts the metadata block: a mutant stripping the Epic, Persona and Status lines from artifact._graft's full render survives at base and after BG0773, and the product renders a bare '> **Epic:** EP0001' where the template has the linked form. Found by BG0773's QA review; pre-existing since delivery [LC-002].
- **artifact batch prints template=minimal when a project story template is in force**: With a project-declared story template, artifact.py batch reports template=minimal in its output although the project template was used. Pre-existing; found by BG0755's QA review.
- **verify_ac stamps --staged passes a -k selector with a term that no longer matches**: verify_ac.py stamps --staged reports every stamped verifier resolving when a pytest -k expression still selects something through its other terms, so a renamed test leaves a dead term behind unseen (US0081 after BG0755). Pre-existing; found by BG0755's round-2 QA review [LC-002].
- **The dropped-content check passes a value that appears elsewhere in the render**: artifact.py's _refuse_dropped_content probes for a supplied value by alphanumeric substring anywhere in the rendered page, so a value that happens to appear elsewhere passes at exit 0 while being dropped (role 'draft' on an epic matches the Status line; role 'x' on a CR). Pre-existing; found by BG0755's and BG0773's QA reviews.
