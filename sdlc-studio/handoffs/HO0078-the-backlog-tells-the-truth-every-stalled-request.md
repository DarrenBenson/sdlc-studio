# HO-0078: the backlog tells the truth: every stalled request and stale finding carries a dated ruling, and the state that let them accumulate cannot rebuild
...
7 of 8 unit(s) remain (1 suit copilot-assisted completion, 6 need human judgement). Plan them straight back in:
...
None: every batch unit is delivered, abandoned, ruled, dropped, parked or awaiting only a signature. Rulings read from RETRO0119's `## Known issues carried` for RUN-01M306PY.

## Appetite

- **Declared:** wall-clock 5760 min, units 64 unit(s)
- **Spent:** 823.5 min, 1 unit(s) terminal
- **Delivered:** 1 unit(s)
