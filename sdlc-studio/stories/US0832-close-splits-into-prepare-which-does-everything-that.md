...
| 2026-09-18 | full suite | `test_autosprint.PrimaryPathTests` still asserted the pre-split contract - close sets `goal-reached` and the run is no longer open - and went red when the close tail stopped ending the run. The module's own suite was green; only the full suite saw it, which is the rule this repo has broken before and broke again here: the tail fix was committed on a selected subset. The test now asserts PREPARE's contract (open, report filed, verdict recorded) and that `sign` REFUSES a run whose units carry no independent critic record, leaving the successful seal pinned on the fixture built to be signable. |
| 2026-09-25 | US0917 | AC2's entailed paths name the review anchor's close-status block, not the sign-off record |
