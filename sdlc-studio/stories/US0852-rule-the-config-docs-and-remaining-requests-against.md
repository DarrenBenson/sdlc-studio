# US0852: rule the config, docs and remaining requests against HEAD
...
**As a** Maya Okafor
**I want** rule the config, docs and remaining requests against HEAD
**So that** CR0591 is delivered by work that can be planned and checked

## Acceptance Criteria

This story rules the **config, docs, platform and the stakeholder track** cluster: 11 of the 38 requests the audit covers
(CR0496, CR0497, CR0515, CR0528, CR0534, CR0535, CR0536, CR0559, CR0560, CR0561, RFC0058). The cluster is the unit of work because judging whether one of these is still wanted
...
- **Given** the 11 requests above, each read against HEAD
...
- **Verify:** shell python3 -c "import pathlib,re,sys; ids='CR0496 CR0497 CR0515 CR0528 CR0534 CR0535 CR0536 CR0559 CR0560 CR0561 RFC0058'.split(); rows=[]; [rows.append(m) for i in ids for p in pathlib.Path('sdlc-studio').rglob(i+'*.md') for m in re.findall(r'^\\| 2026-\\d\\d-\\d\\d \\| audit ruling \\| (.+)$', p.read_text(encoding='utf-8'), re.M) if 'already delivered' in m.lower()]; print('already-delivered rulings checked:', len(rows)); bad=[m[:60] for m in rows if not re.search(r'\\b(US|BG|CR)\\d{4}\\b', m)]; print('unnamed:', bad) if bad else None; sys.exit(1 if bad else 0)"
- **Verified:** yes (2026-09-21)
