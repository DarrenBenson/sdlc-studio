...
**I want** gate --release green on the whole workspace, the CHANGELOG 5.0.0 section cut from the fragments, and the tag refused unless the gate was green on the tagged commit
**So that** the release is cut deterministically and a tag can never assert a green measured on a different tree
...
- **Verify:** manual run `gate.py --release` at release time and confirm it is green (it cannot be a per-story Verify - `--release` runs every story's Verify, so a story whose Verify is `gate --release` would recurse; the operator/CI runs it once before tagging)
- **Verified:** manual (2026-07-26 - gate --release run by hand on the release commit before tag)
