...
- **AC2:** Given the refreshed `trd.md` and `tsd.md`, then neither states a count of scripts, modules, files or tests outside its Revision History (today: "60+ scripts", "(6 modules)", "well over 2,500 tests", the TSD's "90+ modules"), and neither carries a `<!-- measured: ... -->` timing marker, whose only reader (`check_spec_claims.py`) US0879 deleted; red against today's TRD and TSD
