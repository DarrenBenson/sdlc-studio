...
| 2026-08-07 | sdlc-studio | Delivery review round 1 REJECTed. The close's survivor row was not run-scoped at all - it counted every survivor ever filed while its own title, the criterion and the changelog said THIS RUN - so the run id is now stamped at filing and the count filters on it. `test_a_survivor_bug_never_parents_another` was vacuous: the child close exited 1 before the guard, and the test never asserted the exit code. `_survivor_severity` stated signals FALSE of the body it read - an if/else returning on both arms derived a None path that does not exist, and a nested helper's `raise` was read as its enclosing reporter's - so terminality is now recursive and the walk stops at nested scopes. `--force` no longer skips filing, and the idempotence key is hashed and searched recursively |
| 2026-08-07 | sdlc-studio | Delivery review round 2 ruled the run-scoping CLOSED and two repairs MOVED. The never-parents test was STILL vacuous, for a second reason: the child bug had no surviving mutant, so nothing would be filed with or without the guard - it now has one, asserted before the close runs. `_terminates` handled If, Try, With and while-True but not Match, For-else or AsyncWith, so the false signal reappeared one construct over, and an inner loop's break counted as the outer's; every fixture now carries its own line, because a fixed line probed past the end of a short body and got `module level`, a different wrong answer wearing the same tick. The run stamp, `--force`, the hashed key and the recursive lookup each had no cover and now do, and a survivor filed with no run open is REPORTED rather than dropped |
| 2026-08-07 | sdlc-studio | Delivery review round 3 APPROVEd, ruling all five round-2 findings CLOSED. Its residue is fixed rather than carried: a `break` in a for-else's body jumps PAST the else, so a body that can break has an escape the else never sees and the terminality rule was still one case wrong. `HONEST_HIGH` is new beside `HONEST_MEDIUM` - without bodies that genuinely DO yield None, the rule can be widened until it claims everything terminates and every Medium fixture still passes. The orphan report, the match catch-all and the for-else conjunct each have cover now |
