...
- **Given** a project with a custom terminal status in its status vocabulary
- **When** `backlog` classifies rows
- **Then** it resolves terminal statuses from the shared vocabulary, so a custom terminal status is excluded without a code change
