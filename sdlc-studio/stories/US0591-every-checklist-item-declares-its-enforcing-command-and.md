...
> **Affects:** .claude/skills/sdlc-studio/scripts/sprint_report.py, .claude/skills/sdlc-studio/scripts/tests/test_sprint_report.py, .claude/skills/sdlc-studio/scripts/sprint.py, .claude/skills/sdlc-studio/scripts/tests/test_sprint.py
> **Epic:** EP0197
> **Points:** 5
...
- **Verify:** pytest .claude/skills/sdlc-studio/scripts/tests/test_sprint_report.py::ChecklistWindowTests::test_the_shipped_checklist_command_reports_the_expired_item_and_does_not_refuse_on_it
- **Verified:** yes (2026-08-07)

### AC3: a close-window item still gates

- **Given** a close reached with no full-diff review recorded - a `sprint close` window item,
  unsatisfied
- **When** the checklist resolves
- **Then** it is OUTSTANDING and holds the close - the control, so moving windows does not
  disarm the items the close genuinely owns. The criterion originally named an unwritten retro;
  that row reports `ran` for a missing file, which is BG0540 and not this unit's to fix, so the
  example moved to a row that gates as the table intends
