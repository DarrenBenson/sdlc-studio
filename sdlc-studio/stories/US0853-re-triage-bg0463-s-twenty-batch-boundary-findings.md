...
- **Verify:** shell python3 -c "import pathlib,re,sys; buckets=[q for q in pathlib.Path('sdlc-studio').rglob('*.md') if re.search(r'(?m)^> \\*\\*Consolidation:\\*\\*', q.read_text(encoding='utf-8'))]; bad=[q.name for q in buckets for ln in q.read_text(encoding='utf-8').splitlines() if ln.startswith('- ') and 'BG0463 claim' in ln]; print('survivors still carried as bullets inside:', bad) if bad else None; sys.exit(1 if bad else 0)"
- **Verified:** yes (2026-09-21)
- **Verified:** yes (2026-09-21) - all 15 survivors are their own artefacts
...
| 2026-09-21 | delivery | AC2 is now MET, and honestly: the two remaining survivors are minted as BG0734 and BG0735 through `artifact.py new` - not a severity inflated to dodge the Low-consolidation mechanism, and not the mechanism switched off - and removed from CR0592. The verifier is also tightened: it scanned the whole file for `BG0463`, so this very revision row would have failed it, which is a verifier measuring prose rather than the thing the criterion is about. It now checks FINDING BULLETS only. The `Verified:` line returns to yes because the criterion is genuinely satisfied, not because the gate was persuaded. |
