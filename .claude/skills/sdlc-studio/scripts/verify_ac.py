...
def duplicate_verifiers(paths) -> list[dict]:
    """Verify commands shared by more than one criterion of ONE artefact.

    Two criteria of one unit sharing a selector cannot both be discriminating: a regression in
    either behaviour fails both, and neither tells you which one broke. It is also how a
    ...
    Judged within one artefact only. A bug naming its fixing story's selector is correct
    sharing - the bug's fix IS the story's criteria - so two artefacts naming one run are never
    a group. Reported per shared command with every AC that claims it, as advice: `lint` prints
    it and the review brief notes it, and neither refuses anything.
    """
    ...
    for p in paths:
        ...
        for block in criteria_blocks(text):
            ...
            seen.setdefault((rec, expr), []).append(f"{rec} {block.ac_id}")
            written.setdefault((rec, expr), set()).add(" ".join(block.verifier.split()))
    out = []
    for key, acs in sorted(seen.items()):
        if len(acs) < 2:
            continue
        ...
        spellings = sorted(written.get(key) or [key[1]])
...
def dup_group_key(verifier: str) -> str:
    """The identity a shared-selector group is keyed on: whitespace normalised, VERB lowercased.
...
def cmd_lint(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "bugs", False) and not args.story:
        # `sdlc-studio/bugs` too, as `stamps --bugs` already scans it.
...
#: is deliberately outside it - a selector never names one.
...
def cmd_stamps(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "staged", False):
        try:
            rc, lines = staged_stamps(repo_root)
        except StagedGitError as exc:
            print(f"verify-stamps: staged: refused - {exc}", file=sys.stderr)
            return 2
        for line in lines[:-1]:
            print(line)
...
def revert_exemptions(text: str) -> dict:
    ...
    One class: a DECLARED `Revert-check-exempt` field carrying a reason with substance. A bare
    id list is malformed and exempts nothing, because a state that costs nothing to enter is
    the state every awkward criterion chooses. A legacy `## Test Plan` row is plain text and
    exempts nothing: a unit's criteria and their Verify selectors are its whole test plan.
    """
    out: dict = {}
...
RETIRED_VERBS = {
    ...
    "testplan": "no `## Test Plan` is derived, probed or ruled on any more, because the "
                "criteria and their Verify selectors are the plan",
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    st.add_argument("--staged", action="store_true",
                    help="judge only the stamped selectors naming a STAGED test file, against "
                         "the index blob by AST - the pre-commit lane; a commit staging no test "
                         "file reads nothing")
    ...
    ln.add_argument("--bugs", action="store_true",
                    help="scan sdlc-studio/bugs too, as `stamps --bugs` does")
