...
class UncommittedSurfaceTests(unittest.TestCase):
    ...
    def test_the_reason_names_the_isolated_checkout_route(self) -> None:
        """A reason that names the problem and no route is a complaint (US0936: the hand route
        recorded its verdict with `register`, which is retired). Mutant: drop the route, or put
        the retired verb back - a reader is then sent to a command that refuses."""
        m = _load()
        ...
        self.assertIn("commit or stash", reason, "the way to clear the refusal is missing")
        self.assertIsNone(re.search(r"mutation\.py (register|retract|retractions|audit)\b",
                                    reason), f"the reason names a retired verb: {reason}")
...
class UncommittedSurfaceCLITests(unittest.TestCase):
    ...
    def test_the_refusal_names_the_isolated_checkout_route(self) -> None:
        """A refusal that names the problem and no route is a complaint (US0936: the hand route
        recorded its verdict with `register`, which is retired). Mutant: drop the route from the
        printed remedy, or put the retired verb back - a reader is then sent to a command that
        refuses."""
        m = _load()
        with tempfile.TemporaryDirectory() as d:
            root = self._repo(d, dirty=True)
            _code, out = _mut_cli(m, "run", "--root", str(root),
                                  "--files", str(root / "src" / "thing.py"), "--test", "true")
            self.assertIn("worktree add", out, f"the isolated-checkout route is missing:\n{out}")
            self.assertIsNone(re.search(r"mutation\.py (register|retract|retractions|audit)\b",
                                        out), f"the refusal names a retired verb:\n{out}")
