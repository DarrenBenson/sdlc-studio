...
## Shared Verify selectors

Two criteria of one artefact sharing a Verify selector cannot both discriminate: a regression in
either fails both, and neither tells you which broke. `lint` names each such pair, and the review
brief carries one advisory line per pair, so the seat judges whether the two assert one
indivisible behaviour or need splitting. Neither refuses anything.

```bash
python3 "$CLAUDE_SKILL_DIR/scripts/verify_ac.py" lint --bugs
```

The pair is judged within one artefact only. A bug naming the selector of the story that fixed it
is correct sharing - the bug's fix is the story's criteria - and is never reported.
