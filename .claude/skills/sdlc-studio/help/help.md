...
The catalogue follows the process spine: **raise** work into the Discovery backlog, **break it
down** into the Delivery backlog, then **sprint and review** it to done. The document levers
steer all three. Support keeps the backlog honest and visible; utility covers the lifecycle
around the spine.

### Raise

Intake into the Discovery backlog: a defect, a proposed change, an unsettled design, or a raw
report that has not been diagnosed yet.
...
| `/sdlc-studio cr sync` | Two-way sync CRs with GitHub Issues (push + pull) |
...
Discovery to Delivery. A request is **refined** into an epic + stories; an Issue is **triaged**
into bugs. Under `two_backlog.enforce`, `sprint plan` refuses a Discovery item, and its terminal
status derives from its children.
...
**State tracking:** `story implement` creates `sdlc-studio/workflows/WF{NNNN}.md` to track
progress across sessions.

### Sprint and Review

The delivery loop, and the independent look at what it produced.
...
The top-level human direction documents, and the personas that speak for them. These steer
everything the spine produces, so they come before the tooling.
...
Keep the backlog honest and visible: what state things are in, what drifted, what the evidence
says, and what the last run taught.
...
**Four Pillars** (what `status` reports):
...
Legitimate lifecycle work that sits off the delivery spine: setting a project up, writing and
running code and tests, migrating artefacts, shipping, and closing a batch out.
...
| `/sdlc-studio project sync` | Sync all three types (cr, story, epic) with GitHub Issues |
...
**Retro dispositions.** Every finding takes one: **filed** (a `BG`/`CR` id) or **declined with a
reason**. Both are green - honesty costs exactly what noise costs. Silence does not pass.

**External sync.** Two-way sync between local records and external trackers (`cr sync` under
Raise, `story sync` under Break Down, `project sync` above). v1.6.0 ships GitHub Issues; Linear,
Jira, and GitHub Projects board integration are deferred. Requires `gh` CLI installed and
authenticated. See `reference-github-sync.md`.

**State tracking.** `project implement` creates `sdlc-studio/.local/project-state.json` for
cross-epic progress.

**Folded commands.** Out of the catalogue, still routed - the front door named below does the
same work:

- Folded: `/sdlc-studio upgrade` -> `/sdlc-studio migrate` - the schema-v1 doc-shape upgrade is one of the steps `migrate` orchestrates, alongside conventions, version and sizing. See `help/upgrade.md` and `reference-upgrade.md#three-upgrades`.
