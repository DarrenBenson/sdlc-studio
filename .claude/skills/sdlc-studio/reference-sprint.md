...
6. **Closing gate - the sprint review.** Every run ends with a mandatory
   ...
   **The whole close is one command:** `sprint.py close [--retro RETROxxxx] [--goal-verdict ... --note ...]`
   sequences goal-verdict, retro validate + extract, lessons summary, the close gate, handoff and reconcile
   with fail-loud stops (each naming its remedy; a re-run resumes idempotently). It then **draws the sprint report**
   immediately before the sign-off decision brief, so what the sprint delivered and cost sits on the page the
   reviewer of record judges. `report.enabled: false` gates only that drawing: the chain, the brief and the exit
   code are unchanged, and a report that cannot be composed is noted rather than allowed to fail the close
   (`sprint report --id RETROxxxx` draws it any time). **Run close with no `--retro` first and it
   scaffolds the retro through the deterministic path** (`artifact.py new --type retro`: allocated id, template
   and index row, Batch/Goal pre-filled from the run) and stops so you fill it, then re-run with the id it prints.
   This is the only path that also wires the retro's index row - a hand-authored retro lands a missing-row the
   reconcile step catches last; a named-but-missing `--retro` is refused (the allocator cannot mint a chosen id).
   Judgement stays outside: the retro's content, the verdict's note and the signature remain human/agent work.
...
9. **The compulsory checklist (a gate, not doctrine).** Every stage above is a ROW of the sprint
   ...
   added to the cycle with no row fails `sprint_report.cycle_drift()`, which walks every script
   the rows name and reports a row it could not check as unverifiable rather than green - so
   the two cannot part without something saying so.
