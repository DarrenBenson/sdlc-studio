...
**Reading Guide** - 838 lines, 21 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.
...
| Invocation | 48-75 |
| The loop | 76-327 |
| Definition of Done | 328-343 |
| Goals | 344-356 |
| &nbsp;&nbsp;`--goal plan` - sprint planning | 357-427 |
| &nbsp;&nbsp;`--goal design` - establish the dependency graph | 428-438 |
| The breakdown gate - `plan` refuses an ungroomed batch {#breakdown} | 439-492 |
| What the plan critic cannot see | 493-515 |
| Authoring mode - greenfield, from a PRD | 516-548 |
| The single-writer rule {#single-writer} | 549-574 |
| Guardrails | 575-594 |
| Autonomous mode (`--autonomous`) | 595-615 |
| The Two Instruments: Schedule and Cost {#schedule-and-cost} | 616-658 |
| Appetite (the run-level circuit breaker) {#appetite} | 659-712 |
| Rolling multi-sprint policy {#rolling} | 713-738 |
| Model-tier routing {#model-tier-routing} | 739-771 |
| Scripts | 772-791 |
| In-flight controls {#in-flight-controls} | 792-817 |
| Nothing is fixed during a close {#close-fixed-point} | 818-833 |
| See Also | 834-838 |
...
6. **Closing gate - the sprint review.** Every run ends with a mandatory
   ...
   **One full-diff pass is recorded once, as coverage:**
   ...
   unit** (`sprint.py sign` fans it), the brief reading it reviewed. The closing gate also emits the
