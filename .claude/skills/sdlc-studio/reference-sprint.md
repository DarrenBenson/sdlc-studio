...
<!-- BEGIN GENERATED reading-guide -->
**Reading Guide** - 839 lines, 21 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.

| Section | Lines |
| --- | --- |
| Precondition - a runnable gate (cold start) | 38-47 |
| Invocation | 48-75 |
| The loop | 76-328 |
| Definition of Done | 329-344 |
| Goals | 345-357 |
| &nbsp;&nbsp;`--goal plan` - sprint planning | 358-428 |
| &nbsp;&nbsp;`--goal design` - establish the dependency graph | 429-439 |
| The breakdown gate - `plan` refuses an ungroomed batch {#breakdown} | 440-493 |
| What the plan critic cannot see | 494-516 |
| Authoring mode - greenfield, from a PRD | 517-549 |
| The single-writer rule {#single-writer} | 550-575 |
| Guardrails | 576-595 |
| Autonomous mode (`--autonomous`) | 596-616 |
| The Two Instruments: Schedule and Cost {#schedule-and-cost} | 617-659 |
| Appetite (the run-level circuit breaker) {#appetite} | 660-713 |
| Rolling multi-sprint policy {#rolling} | 714-739 |
| Model-tier routing {#model-tier-routing} | 740-772 |
| Scripts | 773-792 |
| In-flight controls {#in-flight-controls} | 793-818 |
| Nothing is fixed during a close {#close-fixed-point} | 819-834 |
| See Also | 835-839 |
<!-- END GENERATED reading-guide -->
...
4. **Per unit, in order:**
   ...
   - **Independent critic (D3), scaled to stakes** - the review depth matches the unit's risk, so tokens are spent in
     ...
     **The operator signs once, for the run:** a unit at **Review** with an independent delivery APPROVE owes only the
     run's signature. `sprint.py sign` moves each batch unit to its terminal status and writes one signature, from a
     principal the author does not control; no per-unit sign-off row is written. It stops before moving anything on a
     unit with no independent APPROVE. The close names the same units first: one at Review as a known issue, one
     already moved to Done in the pre-flight's blocking `review-coverage` row.
...
5c. **The bounded close exit.** A blocked close is offered a choice, not a treadmill: fix
   ...
   ADMINISTRATIVE blocker (ceremony debt: an unjudged goal or a retro gap) is filed as a
   real artefact linked to the run, the deferrals are named in the retro and the review
   anchor, and the run closes with outcome `closed-outstanding` - stated plainly, nothing
   waived. A HARD blocker (a red gate lane, a refusing Done gate)
   refuses the whole exit, and so does an UNANSWERED batch unit - unfinished and unruled, or
   carrying a standing REJECT: filing is for ceremony debt, never for a failing test, and a
   stop-ship question is ANSWERED, never filed. The refusal names each unit and its ways out,
   the operator's ruling in the retro's `## Known issues carried` table among them; `stop
   --force` ends the run over them as a recorded waiver that answers none. Every close attempt
   also records its outstanding count, and a re-run reports the trend (shrinking / growing),
   so a spiral is visible rather than inferred.
6. **Closing gate - the sprint review.** Every run ends with a mandatory
   ...
   **Mutation evidence first (mechanical, `--goal done` only):** before the retro gate, run
   ...
   diff. Survivors are findings for the critic pass below, not an automatic block; no gate
   lane reads the report. For `--goal design` it reviews the produced
   ...
   **Each unit's verdict is recorded once, with `critic.py record`:** one verdict ledger says
   whether a unit was reviewed. A historical sprint-level row, dated before the batch-review
   verbs were retired, still reads as coverage for a unit with no verdict of its own - never
   overriding a per-unit REJECT. The operator's **one signature seals the run** (`sprint.py
   sign`), the brief reading it reviewed. The closing gate also emits the
...
9. **The compulsory checklist (a gate, not doctrine).** Every stage above is a ROW of the sprint
   ...
   as a conformance waiver. The handoff row is reported and never held - the close produces it
   itself, and a gate whose only exit is the step it blocks is a deadlock. A stage
