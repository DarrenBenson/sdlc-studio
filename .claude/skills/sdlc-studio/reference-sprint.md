...
**Reading Guide** - 836 lines, 21 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.
...
| Invocation | 48-75 |
| The loop | 76-325 |
| Definition of Done | 326-341 |
| Goals | 342-354 |
| &nbsp;&nbsp;`--goal plan` - sprint planning | 355-425 |
| &nbsp;&nbsp;`--goal design` - establish the dependency graph | 426-436 |
| The breakdown gate - `plan` refuses an ungroomed batch {#breakdown} | 437-490 |
| What the plan critic cannot see | 491-513 |
| Authoring mode - greenfield, from a PRD | 514-546 |
| The single-writer rule {#single-writer} | 547-572 |
| Guardrails | 573-592 |
| Autonomous mode (`--autonomous`) | 593-613 |
| The Two Instruments: Schedule and Cost {#schedule-and-cost} | 614-656 |
| Appetite (the run-level circuit breaker) {#appetite} | 657-710 |
| Rolling multi-sprint policy {#rolling} | 711-736 |
| Model-tier routing {#model-tier-routing} | 737-769 |
| Scripts | 770-789 |
| In-flight controls {#in-flight-controls} | 790-815 |
| Nothing is fixed during a close {#close-fixed-point} | 816-831 |
| See Also | 832-836 |
...
6. **Closing gate - the sprint review.** Every run ends with a mandatory
   ...
   **One full-diff pass is recorded once, as coverage:**
   ...
   overriding a per-unit REJECT (repaired per unit), the operator's **one signature sealing the
   run** (`sprint.py sign`), the brief reading it reviewed. The closing gate also emits the
