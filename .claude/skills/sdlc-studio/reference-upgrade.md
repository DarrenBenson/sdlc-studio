...
**Reading Guide** - 541 lines, 30 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.

| Section | Lines |
| --- | --- |
| The v5 gate delta - read the consuming page first | 42-54 |
| Contents | 55-69 |
| The commands called "upgrade" {#three-upgrades} | 70-85 |
| migrate - one pass over everything {#migrate} | 86-122 |
| Schema Versions | 123-131 |
| Two-backlog model migration {#two-backlog-migration} | 132-158 |
| Version Check (hint and status only) | 159-162 |
| &nbsp;&nbsp;Dismissal File | 163-178 |
| Version Detection | 179-180 |
| &nbsp;&nbsp;On Any Command | 181-191 |
| &nbsp;&nbsp;Upgrade Suggestion Output | 192-200 |
| /sdlc-studio upgrade - Step by Step {#upgrade-workflow} | 201-202 |
| &nbsp;&nbsp;1. Parse Arguments | 203-210 |
| &nbsp;&nbsp;2. Detect Current Version | 211-222 |
| &nbsp;&nbsp;3. Inventory Existing Artifacts | 223-240 |
| &nbsp;&nbsp;4. Transform Each Artifact | 241-297 |
| &nbsp;&nbsp;5. Preserve During Transform | 298-314 |
| &nbsp;&nbsp;6. Update Index Files | 315-323 |
| &nbsp;&nbsp;7. Write Version File | 324-335 |
| &nbsp;&nbsp;8. Generate Report | 336-369 |
| Dry Run Output | 370-401 |
| /sdlc-studio project upgrade - convention migration {#project-upgrade-workflow} | 402-413 |
| &nbsp;&nbsp;Re-baseline: in-flight artefacts vs the capability delta {#rebaseline} | 414-468 |
| &nbsp;&nbsp;v3.1 default amigo cards (seat-aware, greenfield-only) | 469-495 |
| Backward Compatibility | 496-497 |
| &nbsp;&nbsp;v2 Skill Reading v1 Artifacts | 498-506 |
| &nbsp;&nbsp;Upgrade Is Optional | 507-515 |
| Configuration Upgrade | 516-526 |
| Rollback | 527-536 |
| See Also | 537-541 |
...
3. **the artefact-review sweep** - every open artefact reviewed: an accepted childless request is
   ...
   a re-size.
4. **retired review surfaces** - a retired `[check: <id>]` tag in the Definition of Ready or Done
   is removed and its criterion kept as a human-judged item; a retired `.config.yaml` key (the
   `plan_review` block, `review.two_role_after`, `review.test_plan_after`, `review.signoff`,
   `review.mutation_evidence`, `review.line_coverage_after`, `review.require_brief_provenance`,
   `review.plan_falsifiability`, `review.repair_plan_gate`, `review.repair_design_threshold`,
   `quality.depth_parity_gate`) is removed with its block's children, every other line kept byte
   for byte and line endings kept. A block the removal leaves empty goes too, since an empty
   block reads as null and hides the shipped defaults under it. A comment directly above a
   removed key is kept and named. Every rewrite is re-parsed with PyYAML and must equal the
   original minus those keys; a layout that cannot be edited line by line (a flow mapping), or
   a machine without PyYAML, gets nothing written and the keys reported. The ids and keys come
   from `sdlc_md.RETIRED_CHECK_IDS` and `sdlc_md.RETIRED_CONFIG_KEYS`.

It emits ONE report, split into **deterministic** (what it upgraded automatically - version,
config, sizing conversions, retired tags and keys) and **needs a human** (each item with the exact
command, and each `AGENTS.md`, `CLAUDE.md`, Definition of Ready or Done line that names a retired
key or verb, by file and line; that text is never rewritten). The review ledgers nothing writes any more
(`plan-review-verdicts.md`, `signoff-record.md`, `repair-record.md`, `critic-evidence.md`,
`sprint-review-record.md`, `plan-rulings.md`) are listed as **frozen history** and left as
written; for `repair-record.md` and `sprint-review-record.md` it counts the rows dated on or after
the repair verb's retirement, which no longer answer a REJECT or cover a unit. The honesty
...
stamp, ACs written before the Verify-line convention). `project upgrade` censuses every
non-terminal artefact and buckets each gap: **backfill** (a deterministic stamp computable now,
applied on `--apply` - e.g. a `Difficulty` band from `route estimate`) and **residual** (judgement
gaps the tooling can only name). Terminal artefacts are never touched.
...
story that pre-dates a gate gates only when it next moves; its past transitions stand. No fabricated history: telemetry and metrics begin at the upgrade, and no
