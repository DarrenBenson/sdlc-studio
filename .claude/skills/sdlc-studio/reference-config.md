...
**Reading Guide** - 714 lines, 37 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.

| Section | Lines |
| --- | --- |
| Contents | 49-77 |
| Configuration Files | 78-85 |
| Configuration Loading | 86-103 |
| Coverage Targets | 104-117 |
| Story Quality Gates | 118-121 |
| &nbsp;&nbsp;Edge Cases | 122-130 |
| &nbsp;&nbsp;Test Scenarios | 131-137 |
| &nbsp;&nbsp;Story Sizing | 138-148 |
| Engagement Floor {#engagement-floor} | 149-164 |
| TDD Trigger | 165-176 |
| E2E Limits | 177-188 |
| Epic Perspectives | 189-204 |
| Review Configuration | 205-220 |
| &nbsp;&nbsp;Review gate cutoffs {#review-cutoffs} | 221-227 |
| &nbsp;&nbsp;The plan falsifiability gate {#plan-falsifiability} | 228-251 |
| Persona Staleness {#personas-staleness-days} | 252-263 |
| Lessons Validity {#lessons-validity} | 264-280 |
| Sprint Breakdown Gate {#breakdown} | 281-289 |
| Two-Backlog Enforcement {#two-backlog} | 290-299 |
| Sprint Capacity {#capacity} | 300-315 |
| Run Appetite {#appetite} | 316-328 |
| Skill Source Repo {#skill-source-repo} | 329-347 |
| Contract Tables {#contract-tables} | 348-363 |
| Release Strategy {#release-strategy} | 364-381 |
| Deploy Contract {#deploy} | 382-410 |
| Model-Tier Routing {#routing} | 411-440 |
| Plan-Review Gate {#plan-review} | 441-464 |
| Spec-Edit Guard {#spec-guard} | 465-487 |
| Using Config in Templates | 488-500 |
| Example Project Config | 501-554 |
| Conventions (tolerant convention layer) | 555-593 |
| Version File | 594-614 |
| &nbsp;&nbsp;Schema version stamp {#schema-version} | 615-635 |
| &nbsp;&nbsp;Schema v3 triage lane {#triage-vocab} | 636-672 |
| Pipeline Profile {#profile} | 673-695 |
| Project Implementation | 696-708 |
| See Also | 709-714 |
...
Asks, of every criterion in a batch, whether it CAN FAIL as written - the question nothing else
in the toolchain asks. A criterion the tree already satisfies states what the tree already does,
and every gate downstream is built on the assumption that somebody checked.

| Setting | Default | Notes |
| --- | --- | --- |
| `review.plan_falsifiability` | `report` | `block` refuses the batch; `off` does not probe at all. Quote `off` - YAML 1.1 reads the bare word as a boolean, and an unrecognised value is refused by name |

`sprint plan` runs each criterion's verifier, so planning costs what those verifiers cost -
about 8-12 seconds per unit on the repository this was measured in. It NEVER executes a
`shell`, `eval` or `http` verifier: those are named `not-probed` and nothing is run for them, so
planning cannot run shell somebody else authored into an artefact.

The finding classes are `green` (already passes), `never-fails` (its every selected test is
skipped) and `unreadable` (the runner could not be trusted to answer). Everything else -
`red`, `not-yet-written`, `manual`, `not-probed`, `delivered`, `unspecified` - is a state a plan
is allowed to be in. Probe a unit on its own with `verify_ac.py testplan probe --unit <id>`, and
answer a finding you mean to plan over with `verify_ac.py testplan rule` (withdraw it later with
`verify_ac.py testplan withdraw`); a ruled criterion is reported and never refused.
