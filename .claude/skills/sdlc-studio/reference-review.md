...
**Reading Guide** - 844 lines, 38 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.

| Section | Lines |
| --- | --- |
| Contents | 50-65 |
| Overview | 66-75 |
| When to Run a Unified Review | 76-96 |
| /sdlc-studio review - Step by Step {#review-workflow} | 97-98 |
| &nbsp;&nbsp;1. Parse Arguments | 99-119 |
| &nbsp;&nbsp;2. Run Document Reviews | 120-198 |
| &nbsp;&nbsp;3. Cross-Document Consistency Check | 199-273 |
| &nbsp;&nbsp;3a. Persona Consultation (default when personas exist) | 274-291 |
| &nbsp;&nbsp;3b. Auto-Apply Mechanical Fixes | 292-319 |
| &nbsp;&nbsp;4. Close the Review (tool-carried) | 320-345 |
| &nbsp;&nbsp;5. Generate Consolidated Report | 346-401 |
| The adversarial closing-review brief {#closing-review-brief} | 402-410 |
| &nbsp;&nbsp;The claim inventory runs first | 411-432 |
| &nbsp;&nbsp;The standing review practices | 433-460 |
| &nbsp;&nbsp;A REJECT has two exits | 461-480 |
| &nbsp;&nbsp;Where a delegated reviewer mutates {#mutation-isolation} | 481-516 |
| Correcting the verdict log {#superseding-a-verdict} | 517-553 |
| &nbsp;&nbsp;What a superseded row does to the gates | 554-569 |
| Quick Mode | 570-588 |
| Focus Mode | 589-641 |
| TSD Review Workflow (Detailed) {#tsd-review-workflow} | 642-643 |
| &nbsp;&nbsp;1. Load TSD | 644-652 |
| &nbsp;&nbsp;2. Analyse Current Testing State | 653-671 |
| &nbsp;&nbsp;3. Identify Gaps | 672-682 |
| &nbsp;&nbsp;3a. Validate Test Organisation Tree | 683-696 |
| &nbsp;&nbsp;4. Update TSD | 697-707 |
| &nbsp;&nbsp;5. Report Strategy Currency | 708-719 |
| Cross-Document Rules | 720-721 |
| &nbsp;&nbsp;PRD → TRD Mapping | 722-729 |
| &nbsp;&nbsp;TRD → TSD Mapping | 730-737 |
| &nbsp;&nbsp;PRD NFR → TSD Gate Mapping | 738-746 |
| &nbsp;&nbsp;PRD → Persona Mapping | 747-756 |
| &nbsp;&nbsp;Persona → Artefact Activity Mapping | 757-768 |
| Review Output Formats | 769-770 |
| &nbsp;&nbsp;Console Output (Default) | 771-774 |
| &nbsp;&nbsp;JSON Output (CI/CD) | 775-807 |
| A disclosed sign-off is not an independent one | 808-836 |
| See Also | 837-844 |
...
### A REJECT has two exits

A delivery REJECT is answered only by a **round-2 APPROVE from the reviewer who rejected**, who
re-checks the fixes. A round-2 REJECT at the cap (`review.max_rounds`, 2) instead **carries the
unit**: `critic.py record` files the findings as a bug and drops the unit from the open run's
batch, so the close no longer holds it and the run continues. Carrying releases the run, not
the REJECT: the carried unit is still refused Done. It is delivered again in a later run and
...
A licence for the census. `conformance.py check` reads a story already at Done over a delivery
REJECT dated before `critic.REPAIR_VERB_RETIRED` (2026-09-26, the first day no repair row
could answer one) as critiqued, and counts it on its own `LICENSED critiqued` line. The key is the
REJECT's own date alone: the old `repair-record.md` is read by nothing, so the licence also
passes a REJECT that no repair row ever answered. A recorded waiver keeps its attribution over
it. The Done guard, the seal's review bar, the close and the sprint report never read that date,
so a unit in a run still needs one of the exits above.
