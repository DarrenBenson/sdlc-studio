...
**Reading Guide** - 471 lines, 12 section(s). Generated by `docgen.py reading-guides`; read a span with `Read(offset, limit)` rather than the whole file.

| Section | Lines |
| --- | --- |
| /sdlc-studio trd create - Step by Step {#trd-create-workflow} | 30-120 |
| /sdlc-studio trd generate - Step by Step {#trd-generate-workflow} | 121-259 |
| /sdlc-studio trd review - Step by Step {#trd-review-workflow} | 260-305 |
| /sdlc-studio trd visualise - Step by Step {#trd-visualise-workflow} | 306-365 |
| Component Constraints {#component-constraints} | 366-382 |
| Diagram Guidelines {#diagram-guidelines} | 383-384 |
| &nbsp;&nbsp;Element Limits | 385-392 |
| &nbsp;&nbsp;Naming Conventions | 393-400 |
| &nbsp;&nbsp;Visual Hierarchy | 401-410 |
| &nbsp;&nbsp;Connection Labels | 411-418 |
| &nbsp;&nbsp;Subgraphs | 419-451 |
| Navigation {#navigation} | 452-471 |
...
10. **Write TRD**
    ...
    - Fill the Component Overview Constraints column (see [Component Constraints](#component-constraints))
...
The Component Overview table carries a Constraints column: the rule a change to that component
must keep, or `-` when there is none. `trd create` and `trd generate` fill it alike.

`sprint lane brief` reads the column. For each unit it lists every row whose component names a
file in the unit's `Affects`, so the agent building the unit is handed the rule where it can
prevent a defect. When no row matches, the TRD has no Constraints column, or there is no TRD, the
brief says so in one line and the dispatch goes ahead.

- Name each component by its path in backticks: `scripts/` for a directory, `SKILL.md` or
  `help/*.md` for files. A path is matched by whole segments, so `scripts/` names
  `app/scripts/a.py` and never `tools/scripts_helper.py`; a component with no backticked path
  matches nothing.
- State a rule that governs one component in its cell, once. A rule that spans components stays
