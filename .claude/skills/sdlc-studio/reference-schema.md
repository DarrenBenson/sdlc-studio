...
| story | `Epic`, `Persona`, `Serves`, `Owner`, `Reviewer`, `Estimated-by`, `Delivered-by`, `Created`, `GitHub Issue`, `Affects`, `Points`, `Depends on`, `Verification depth`, `Revert-check-exempt` |
| bug | `Severity`, `Priority`, `Points`, `Estimated-by`, `Delivered-by`, `Reporter`, `Assignee`, `Created`, `Verification depth`, `Revert-check-exempt`, `Affects` |
...
Two fields on a delivery unit GATE a check rather than describe the artefact, so what they
may contain is part of the contract:

- **`Verification depth`** opens with a tier token, which `transition -> Fixed` reads. Its
  counted half sits inside `[[derived: ... | fp <hash> ]]` delimiters, is generated by
  `verify_ac.py depth --unit <id> --write`, and is refused when hand-edited - the fingerprint
  seals the span's own text, so the check needs no ledger and works in a fresh clone. Anything
  outside those delimiters is the author's judgement and is carried through untouched.
- **`Revert-check-exempt`** names the criteria a production-only revert may legitimately leave
  green - `> **Revert-check-exempt:** AC2, AC4 - <why>`. It requires a reason with substance;
  a bare id list exempts nothing, on `unnameable`'s precedent that a state costing nothing to
  enter is the state every awkward criterion chooses.
