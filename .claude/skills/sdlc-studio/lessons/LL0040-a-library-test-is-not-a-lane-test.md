...
id: LL0040
title: A library test is not a lane test
tags: [review, testing]
added: 2026-07-22
origin: RUN-01KY3MFX
---

**Lesson.** Pin the surface a USER meets - exit code AND printed text - not only the function beneath it. RUN-01KY3MFX fixed a false-clean in the engagement floor, mutation-tested it, and wrote 'mutation-proven' into the bug. All three mutants stopped at the library functions; none entered the renderer the pre-commit hook actually runs. Mutating that renderer's guard to `if False:` left 3,891 tests green while the lane printed the original defect verbatim.
