...
id: LL0051
title: Apply the FIX as a mutant: if tests fail, they were pinning the defect
tags: [testing, mutation, review, repair]
added: 2026-08-01
origin: sdlc-studio
---

**Lesson.** The usual mutation question is 'does a test fail when I break the code?'. The inverse is sharper and almost never asked: **apply the CORRECT behaviour as a mutant and see whether the suite goes red.** If it does, those tests were not holding the feature - they were pinning the bug.

Found by an independent seat reviewing a repair to `_awaits_signoff`. The repair checked one half of a two-half gate. The reviewer patched in the corrected rule - require the adversarial evidence half as well - and **5 of the 10 tests in the owning class failed**. Every fixture in that class had been built without recording evidence, so the class asserted that a unit still owing its adversarial pass was 'finished bar a signature'. One test was even named `test_a_stop_is_not_refused_when_only_signatures_are_outstanding` for a state its own fixture never created.

Ordinary mutation could not have found this. Breaking the code made those tests fail, exactly as they should - they were internally consistent, just consistent with the wrong rule. Only mutating TOWARDS correctness exposed it.

Use it whenever a repair's tests were written by the same author in the same sitting, and especially when a test's NAME describes a state you cannot find in its fixture.
