...
- `repair_plan.py` - The repair-plan gate (EP0106 / RFC0053, opt-in via `review.repair_plan_gate`, off by default). A REJECT is answered by a written plan (one entry per finding: the change, the approach, what it might break), reviewed by an independent pass before any code, with the verdict pinned to the findings it answered. `brief` prints the reviewer questions; `record --unit <id> --author <who> --plan-file <json>` writes the plan keyed to the unit it repairs; `review --unit <id> --verdict <APPROVE|REJECT> --reviewer <who>` records the independent verdict (the plan's author refused) under the `repair-plan` plan-review kind; `gate --unit <id>` checks a repair may be recorded. When on, `transition.py` refuses a bug reaching Fixed, or a story whose `Parent` or `Delivers` names a BG/RV reaching Done, without an approved plan under its own id.
...
- `backfill_audit_runs.py` - One-off-shaped migration with a standing guard. `plan` reports which findings name an audit run inside their `Raised-by` prose; `apply` lifts it into an `Audit-run` field, records the lens as explicitly unknown (never guessed), and seeds each run into the register as `backfilled`; `check` is the standing sweep - it exits non-zero when any finding names a run in prose that its field does not carry. A `Raised-by` line naming two run ids is resolved only when the prose says which filed it (`run <B>` versus `<A> carry-over`), and refused otherwise.
