...
- **R3 - Duplicate-customer detection.** A candidate customer is a
  duplicate of a stored customer when the names are an exact match and
  the emails match case-insensitively, ignoring surrounding
  whitespace. Emails are normalised at comparison time only; stored
  values are never altered (see R8).
