...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("set-mode", help="switch a user's delivery mode")
    p.add_argument("user_id")
    p.add_argument("mode", choices=["immediate", "digest"])

    p = sub.add_parser("send-digests", help="flush pending digests")
    p.add_argument("--now")
...
def main(argv: list[str] | None = None, service: NotificationService | None = None) -> int:
    ...
    if args.command == "add-user":
        ...
        service.set_delivery_mode(args.user_id, args.mode)
        print(f"{args.user_id} -> {args.mode}")
    elif args.command == "send-digests":
        made = service.send_digests(now=_parse_now(args.now))
        print(f"sent {len(made)} digests")
