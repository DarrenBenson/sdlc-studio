...
class NotificationService:
    ...
    def send_digests(self, *, now: datetime) -> list[Delivery]:
        """Flush pending digests (NT-142).

        Honours the existing spec: a digest is deferred through the
        user's quiet hours (SPEC R5), the whole flush counts as one
        send against the throttle (SPEC R8), and each flush appends a
        single audit entry (SPEC R9). Undelivered digests stay queued.
        """
        made: list[Delivery] = []
        for user_id, notes in sorted(self.store.pending_digests().items()):
            user = self.store.get_user(user_id)
            ids = tuple(n.notification_id for n in notes)
            if in_quiet_hours(now, user.utc_offset_hours):
                # SPEC R5: defer, do not drop - the queue is left intact.
                self.audit.append(
                    user_id, ids, audit_log.REASON_SUPPRESSED_QUIET_HOURS, now
                )
                continue
            if not self.throttle.allow(user_id, now):
                # SPEC R8: over the daily send limit; keep the queue.
                self.audit.append(user_id, ids, audit_log.REASON_THROTTLED, now)
                continue
            made.append(self._deliver(user, tuple(notes), "digest", now))
            self.store.clear_digest(user_id)
        return made
    ...
    def _route(
        ...
        if not user.subscribed and not note.mandatory:
            ...
            return None
        if note.urgent:
            # SPEC R3: urgent is never suppressed, deferred or batched.
            return self._deliver(user, (note,), "immediate", now)
        if user.delivery_mode == "digest":
            # NT-142: batch non-urgent items for opted-in users. The R7
            # check above has already filtered what they may receive.
            self.store.queue_digest(note)
            return None
        if in_quiet_hours(now, user.utc_offset_hours):
