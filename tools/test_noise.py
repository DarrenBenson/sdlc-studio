...
#: unittest's own output. Checked FIRST - a failure's traceback legitimately contains
#: `ERROR:` and `AssertionError:`, and flagging those would fire on every red run.
_RUNNER = re.compile(
    r"""^(?:
        ...
        | OK(?:\ \(.*\))?
        | FAILED(?:\ \(.*\))?
        | (?:FAIL|ERROR):\ .*            # the runner naming a failing test
        | Traceback\ \(most\ recent\ call\ last\):
        | During\ handling\ of\ the\ above\ exception.*
        | \s.*                           # indented: traceback frames and source lines
        | \w*(?:Error|Exception|Warning):\ .*   # an exception's own repr line
        )$""",
    re.VERBOSE)
...
_LEAK = re.compile(
    r"""^(?:
        ...
        | [a-z][a-z0-9_.-]*:\s.*\b(?:failed|refused|error|warning|not\ found|broken|invalid)\b
