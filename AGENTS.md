...
A SECOND lane binds at the push and release boundaries only, and it is ADVISORY there:
`revert-check` (`gate.py --boundary push|release`) reverts each batch unit's declared production
