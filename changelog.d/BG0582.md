...
- The close chain now reads the rung the run was opened at, as the planner always has. A
  ...
  Scoped to `design` specifically, not to "not `done`". `plan` and `triage` are also non-build
  rungs and their product is not grooming, so judging them by it told a plan run it had produced
  no acceptance criteria and left it unable to use `--file-and-close` either - the same defect,
  one rung over. Those rungs are unchanged.

  The build rung is unchanged, and three of the nine criteria exist only to prove it: a `done` run
